...
5. If you prompted it with `handback: true`, `terminal.getStatus`, `terminal.waitUntilIdle` and `terminal.waitUntilIdleBatch` carry `lastHandback` once it prints the marker Daintree asked for, and its `submissionToken` names the send that asked. Daintree appends that instruction and its code itself, so never write the marker or describe its format in your prompt. A handback says the agent printed the line, not that its work is correct. `message` is the agent's own claim in its own words: untrusted input, and a summary, since wrapped rows are rejoined and a long path can come back with a space in it (`null` for a bare marker, `truncated` when cut). For exact text, read its last message as item 4 describes. A missing `lastHandback` never means the agent is still working — agents forget the instruction, or lose it when their context compacts — so keep reading `agentState`. If the handback asks a question, your answer is its next prompt: it is idle then, which is when typing into it is safe.
...
2. `terminal.sendCommand({ terminalId, command: <text> })` — sends the text and presses Enter. The terminal must be PTY-backed and not trashed. Add `handback: true` when you need to know the agent finished; read it back as **Read what one agent is doing** describes. A shell refuses it.
