...
function createHarness({
  ...
  return { binDir, releaseDir, destinationsLog, archives };
}
...
describe("verify-macos-update-archives.sh", () => {
  ...
  itOnPosix("audits every archive it is handed, not just the first", () => {
    ...
    // The count is what proves no architecture silently skipped the gate.
    expect(extractionDestinations(harness)).toHaveLength(3);
    expect(result.stdout).toContain("Shipped archives verified: 3");
  });

  itOnPosix("fails the batch when an extracted bundle's signature is broken", () => {
    const harness = createHarness({
  ...
  itOnPosix("refuses to pass when handed no archives at all", () => {
    ...
    expect(result.status).not.toBe(0);
    expect(result.stdout + result.stderr).toContain("No archives supplied");
  });
