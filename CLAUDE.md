...
- **Design spot-checks:** to visually review a small UI change, drive it with a temporary local-only Playwright spec (`tmp-*.spec.ts` in the matching `e2e/` bucket) that injects the state, pauses ~4s per state for live eyeballing, and screenshots each state — never commit `tmp-*` specs.
