...
- **Tooltips on focus restoration:** Radix opens tooltips on FOCUS as well as hover, so any surface that hands focus back programmatically — a dropdown/popover closing onto its trigger, a dialog restoring focus — leaves that trigger's tooltip open with the pointer nowhere near it. Fix it LOCALLY: a controlled `open` on the trigger's `Tooltip` plus an `isRestoringFocusRef`, both set from `onCloseAutoFocus` (close the tooltip AND arm the flag), with the Tooltip's `onOpenChange` ignoring opens while the flag is set and the trigger's `onPointerEnter` clearing it so a genuine hover still works. Arm it in `onCloseAutoFocus`, never at close-time — Radix defers focus restoration past the exit animation, so a timer armed when the menu starts closing is racing a stall it needn't. `dismissAllTooltips()` (`src/lib/tooltipDismissRegistry.ts`) is for surfaces that can strand a tooltip ANYWHERE — dialogs — not for a menu that knows exactly which one will reopen; blanket popover wiring was rejected in #11034. The accent focus ring is a separate half: `preventDefault()` the restoration for an OUTSIDE click only (`onPointerDownOutside` sets a ref, `onCloseAutoFocus` reads it), never for an item click — Radix has already unmounted the clicked item, so suppressing there drops focus to `document.body` and strands the keyboard user rather than merely hiding a ring. Reset that ref on open; consuming it in `onCloseAutoFocus` isn't enough, since a close that skips the callback strands it onto the next one. Canonical: `src/components/Layout/AgentButton.tsx` and `src/panels/file-browser/FileBrowserViewOptions.tsx`.
