...
// emits a geometry the reader accepts (four digits per dimension
...
// Digit count is deliberately unbounded here: `isUsableTerminalGeometry` is
...
//
// A COLLAPSED header is refused because the files written before #12442 are
// still on disk: five of the reporter's seven snapshots recorded a 2x1 capture
// grid, and a header the reader honours parks the mirror — and, via the
// renderer's own alignment, xterm — on that grid at every restore. Refusing it
// degrades the file to the v1 meaning, "no capture geometry", so the scrollback
// still replays and nothing adopts a grid no pane ever had.
//
// Only the collapse floor, though. A capture grid is not an estimate of
// anything: it is the width the serialized bytes were ENCODED at, and
// SerializeAddon's wrap markers only decode against it. Refusing a small but
// real capture would replay those bytes at the wrong width and bake broken line
// breaks into the buffer — the #11552 damage, caused by the guard meant to
// prevent damage.
function parseGeometryLine(line: string): TerminalGeometry | null {
  ...
  return isUsableTerminalGeometry(geometry) ? geometry : null;
}
 ...
 * (`isUsableTerminalGeometry`), and an unrepresentable grid costs the width
...
function formatSessionFile(snapshot: SerializedTerminalSnapshot): string {
  if (!isUsableTerminalGeometry({ cols: snapshot.cols, rows: snapshot.rows })) {
