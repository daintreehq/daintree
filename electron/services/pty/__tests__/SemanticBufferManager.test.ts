...
describe("SemanticBufferManager", () => {
  ...
  describe("setMaxLines (retention cap)", () => {
    it("lowering the cap trims the existing buffer to the newest lines", () => {
      const info = createTerminalInfo({
        semanticBuffer: Array.from({ length: 30 }, (_, i) => `line ${i}`),
      });
      const manager = new SemanticBufferManager(info);

      manager.setMaxLines(10);

      expect(info.semanticBuffer).toHaveLength(10);
      expect(info.semanticBuffer[9]).toBe("line 29");
      manager.dispose();
    });

    it("caps future accumulation at the lowered limit", () => {
      const info = createTerminalInfo();
      const manager = new SemanticBufferManager(info);
      manager.setMaxLines(5);

      manager.onData(Array.from({ length: 20 }, (_, i) => `cmd ${i}`).join("\n") + "\n");
      vi.advanceTimersByTime(100);

      expect(info.semanticBuffer.length).toBeLessThanOrEqual(5);
      expect(info.semanticBuffer[info.semanticBuffer.length - 1]).toBe("cmd 19");
      manager.dispose();
    });

    it("a zero cap disables the buffer instead of keeping everything (slice(-0) hazard)", () => {
      const info = createTerminalInfo({
        semanticBuffer: Array.from({ length: 5 }, (_, i) => `line ${i}`),
      });
      const manager = new SemanticBufferManager(info);

      manager.setMaxLines(0);
      expect(info.semanticBuffer).toEqual([]);

      // New output must not re-grow a disabled buffer.
      manager.onData("more\noutput\n");
      vi.advanceTimersByTime(100);
      expect(info.semanticBuffer).toEqual([]);
      manager.dispose();
    ...
    it("ignores negative and non-finite caps", () => {
      const info = createTerminalInfo({ semanticBuffer: ["keep"] });
      const manager = new SemanticBufferManager(info);

      manager.setMaxLines(-5);
      manager.setMaxLines(Number.NaN);

      expect(info.semanticBuffer).toEqual(["keep"]);
      manager.dispose();
    });
  });
});
