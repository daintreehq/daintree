...
async function waitForTerminalIdle(
  ...
  const cleanup = () => {
    for (const off of [unsubscribe, unsubscribeKilled, unsubscribeTrash]) {
      if (!off) continue;
      try {
        off();
      } catch (err) {
  ...
  try {
    const settlement = await new Promise<Settlement>((resolve) => {
      ...
      // Ahead of the snapshot re-read below, both branches of which would speak for
      // this terminal in the wrong terms: a trashed terminal still holds a working
      // snapshot until its TTL expires, and once released it has none at all, so
      // without this a user's own close reports `unknown` rather than `closed`.
...
async function waitForBatchIdle(
  ...
  for (const terminalId of orderedIds) {
    ...
    if (store.isTrashed(terminalId)) {
      ...
      // could have gone idle, or finished, before the user closed it. Read off
      // this terminal's own snapshot, which answers for it alone — a sibling
      // running the same agent type cannot speak for it (#12494).
      tracks.set(terminalId, {
        terminalId,
        ...
        busyState: "idle",
        idleReason: "closed",
        previousBusyState: mapAgentStateToBusyState(snapshot.state),
