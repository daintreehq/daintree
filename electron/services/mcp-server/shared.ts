  ...
  },
  {
    ...
    render() {
      return [
        ...
        "- Refill a slot only once its agent is waiting and its PR is confirmed: `prNumber` in `worktree.list` is a cached hint and null does not prove there is no PR, so confirm with the forge (`forge.getPR` or `forge.listPRs` if available) rather than scraping the agent's screen.",
