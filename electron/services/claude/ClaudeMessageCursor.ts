 ...
 *
 * Stateless: it names the message by its id and a digest of the text before
 ...
 * changed or left the read, which is reported rather than having another
 * message's text spliced in.
 *
...
/** The reader's own cap on a message id; a longer one is not one it minted. */
...
function digestPrefix(text: string, end: number): string {
  return createHash("sha256")
    .update(text.slice(0, end), "utf16le")
    .digest("base64url")
    .slice(0, DIGEST_CHARS);
}

export function encodeMessageCursor(id: string | null, text: string, end: number): string {
  const payload = { v: VERSION, id, end, h: digestPrefix(text, end) };
...
export function decodeMessageCursor(value: string): MessageCursor | null {
  ...
  if (id !== null && (typeof id !== "string" || id.length === 0 || id.length > ID_MAX_CHARS)) {
    return null;
  }
  if (typeof end !== "number" || !Number.isSafeInteger(end) || end <= 0) return null;
  if (typeof h !== "string" || h.length !== DIGEST_CHARS || !BASE64URL.test(h)) return null;
...
export function cursorMatches(cursor: MessageCursor, id: string | null, text: string): boolean {
  return (
    ...
    digestPrefix(text, cursor.end) === cursor.digest
  );
}
