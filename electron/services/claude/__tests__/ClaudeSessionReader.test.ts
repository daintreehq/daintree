...
// A hook in front of `lstat`, so a test can change the filesystem at an exact
// point in the reader's sequence instead of racing it.
...
describe("readClaudeLastMessage — which message", () => {
  ...
  // `/exit` leaves user records behind that are not marked meta. They are the
  // CLI's bookkeeping, not something said to the agent.
  it("does not count a local slash command's echo as conversation", async () => {
    await seed([
      assistant("msg_1", [text("Done.")]),
...
describe("readClaudeLastMessage — unanswered tool uses", () => {
  ...
  // Well under both byte caps, but deep enough that the transport would drop
  // the whole structured result for it.
