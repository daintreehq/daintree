...
export function buildRunbooksAddendum(): string {
  return [
    ...
    "",
    '**Before acting on any request to do something, call it** — before your first `daintree` call, even when it looks simple. Search again only when the user asks for a different task: a step of the current one, a follow-up on it and a notice all run on the runbooks you have. Only chat and "how do I…" questions skip it: answer those from the docs. A question you answer by checking something ("how much usage is left?", "why is it slow?") is a task.',
    "",
