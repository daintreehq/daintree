...
function getAppImageWrapperPath(): string {
  return path.join(getAppImageWrapperDir(), "canopy-cli.sh");
}
...
function generateAppImageWrapper(appImagePath: string): string {
  ...
  return [
    "#!/usr/bin/env bash",
    "# Auto-generated by Canopy — do not edit",
    "set -euo pipefail",
    'TARGET="${1:-.}"',
    'ABSOLUTE_PATH="$(cd -- "$TARGET" 2>/dev/null && pwd -P)" || { echo "canopy: \'$TARGET\' is not a directory" >&2; exit 1; }',
