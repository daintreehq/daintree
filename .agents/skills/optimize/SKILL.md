...
**"No improvement was available" is a successful outcome.** Most optimisation attempts fail. A run that tries six hypotheses, disproves all six, and writes down why is worth more than a run that ships a change with no evidence behind it. Never manufacture an improvement. Reverting everything and reporting an honest zero is a complete, correct run — and so is reverting a hypothesis that did move the number because keeping it would have seriously degraded the system. Either way there is no pull request: revert, report that nothing could be optimised here, and finish.
...
| 9 | Pull request | Push the branch and open the PR against `develop` with the evidence in the body (§Finalization). The squash happened before phase 6 — a rewrite here would orphan every number phase 6 measured | PR URL captured — or skipped, with the branch deleted, when nothing improved or every win cost too much |
...
- Regression → REVERT.
- Improvement, but only by seriously degrading the system (§Fixing guidelines) → REVERT anyway, and record the price alongside the number. The measurement was real and belongs in the report; the change does not belong in the tree.
...
- **A win bought by seriously degrading the system is not a win.** Some numbers can only be moved by breaking something worth more than the number: an architectural invariant in `CLAUDE.md` or `.claude/rules/`, a product invariant, a correctness or safety property the predicate does not happen to cover, a cache with no sound invalidation, a user-visible behaviour change nobody asked for, or a rewrite so much harder to maintain that the next person has to undo it. Judge that cost yourself, write the judgement and its reasoning in the ledger, and **revert** — a measured improvement is a licence to keep a change, not an obligation to keep it. A run whose only available wins were of that kind finishes as §Nothing improved: no pull request, and a report saying what the win would have cost.
...
This covers both shapes of a null result: no hypothesis moved the number, **and** the only changes that moved it were ones you reverted because they would have seriously degraded the system (§Fixing guidelines). Both are complete, correct runs, and neither owes a pull request. Do not open one to show your working, do not open a draft, do not commit a change you judged not worth keeping so that something lands. The report is the deliverable — say plainly that nothing could be optimised here and that the run is done.

`git reset --hard <branch point>`, confirm `git status --porcelain` and `git diff origin/develop` are both empty, then `git switch --detach <branch point>` before `git branch -D <branch>`. Git refuses to delete the branch you are standing on, and plain `git switch develop` fails here too — this run works in a linked worktree and `develop` is checked out in the main one. A run that ends holding a branch it said it deleted has lied in its own report. **No pull request, no commit to `develop`, never an empty or marker commit.** The evidence is the report: the target and its before value, every hypothesis with the measured reason it was rejected — including any that won on the number and were reverted on their cost, with that cost named — and what you would try next or why you believe the number is at its floor.
...
📈 **Result**: <headline, or "no improvement was available", or "the only measured wins cost more than they were worth — all reverted">
...
🔗 **PR**: <URL, or "none — nothing worth keeping, so there is nothing to open">
