...
3. **The guards.** Every other metric the scenario emits. Tolerance by class: **10%** for `duration`, `memory`, `derived-ratio`; **5%** for `count`, `size`, `ratio`. A guard outside tolerance is a cost the human did not authorise — revert, or record explicitly why the trade is right and what it cost. Never keep it and mention it in passing.
...
6. **Check every guard** against its precommitted tolerance. Machine-dependent guards are judged against the same drift **D** as the target: a guard that moved less than D has not been shown to move. Reading a guard as noise on grounds you would not accept for the target is how a win gets bought with an unreported regression.
7. **Decide:**
...
It reconstructs the running order from the arms' own `generatedAt` stamps and refuses three champion runs followed by three candidate ones, an order that never reverses, arms passed out of measurement order, repeated or copied arms, and any arm whose `sourceSha` is dirty or is not the tree you named. Then it rules on three conditions: the candidate won **every** index-paired arm, the median-to-median improvement met the precommitted threshold, and it exceeded the champion-versus-champion drift **D**. Pass `--champ`/`--cand` in measurement order, oldest first.
