...
The host checks every call's arguments against the tool's `inputSchema` before `execute` runs, so each schema declares `required` and `additionalProperties: false`. The tools still check each value themselves — real calendar dates a pattern cannot express, bounded integers, a category grammar — because a test can call `execute` directly, and every query binds its values as parameters, never as SQL text.
