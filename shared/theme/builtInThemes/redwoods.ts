import type { BuiltInThemeSource } from "../builtInThemeSources.js";

export const theme: BuiltInThemeSource = {
  id: "redwoods",
  name: "Redwoods",
  type: "dark",
  builtin: true,
  location: "Redwood National Park, California, USA",
  heroImage: "/themes/redwoods.webp",
  palette: {
    type: "dark",
    surfaces: {
      grid: "#090705",
      sidebar: "#110B08",
      canvas: "#16110D",
      panel: "#1E1612",
      elevated: "#2E221B",
    },
    text: {
      primary: "#E8DCC8",
      secondary: "#9E8B78",
      muted: "#6B5B4F",
      inverse: "#16110D",
    },
    border: "#36241A",
    accent: "#4E9A53",
    accentSecondary: "#5A8B55",
    status: {
      success: "#5A7E52",
      warning: "#BA8A42",
      danger: "#B85C52",
      info: "#6B8287",
    },
    activity: {
      active: "#3CAE54",
      idle: "#5A4E42",
      working: "#3CAE54",
      waiting: "#CFA043",
    },
    overlayTint: "#B48C78",
    terminal: {
      cursor: "#CFA043",
      selection: "rgba(78,154,83,0.20)",
      red: "#D4746C",
      green: "#5EAE6C",
      yellow: "#CFA043",
      blue: "#6BA8C4",
      magenta: "#B07AAE",
      cyan: "#5ABCB0",
      brightRed: "#E8988E",
      brightGreen: "#7DC88A",
      brightYellow: "#DDB866",
      brightBlue: "#8DC4DC",
      brightMagenta: "#C89AC4",
      brightCyan: "#7ACEC8",
      brightWhite: "#E8DCC8",
    },
    syntax: {
      comment: "#7a6e5e",
      punctuation: "#B8A898",
      number: "#d4956a",
      string: "#8abf6e",
      operator: "#7ec0b8",
      keyword: "#B08098",
      function: "#8AABCC",
      link: "#6aadcc",
      quote: "#9e8b78",
      chip: "#72c0a8",
    },
    strategy: {
      shadowStyle: "atmospheric",
    },
  },
  tokens: {
    "activity-approval": "#C9682C",
    "border-strong": "rgba(180,140,120,0.14)",
    "border-subtle": "rgba(180,140,120,0.08)",
    "focus-ring": "rgba(180,150,130,0.20)",
    "scrim-medium": "rgba(9,5,3,0.65)",
    "scrim-soft": "rgba(9,5,3,0.35)",
    "scrim-strong": "rgba(9,5,3,0.78)",
    "scrollbar-thumb": "#5A4E42",
    "scrollbar-thumb-hover": "color-mix(in oklab, #5A4E42 85%, #E8DCC8)",
    "search-highlight-background": "rgba(207,160,67,0.18)",
    "search-highlight-text": "#C49A50",
    "search-match-badge-background": "rgba(207,160,67,0.20)",
    "search-match-badge-text": "#C49A50",
    "search-selected-result-border": "rgba(207,160,67,0.45)",
    "search-selected-result-icon": "#C49A50",
    "shadow-ambient": "0 4px 14px rgba(9,5,3,0.45), 0 1px 3px rgba(9,5,3,0.30)",
    "shadow-color": "rgba(9,5,3,0.55)",
    "shadow-dialog": "0 20px 56px rgba(6,3,2,0.50)",
    "shadow-floating": "0 14px 42px rgba(9,5,3,0.55), 0 4px 12px rgba(9,5,3,0.35)",
    "surface-inset": "#0E0A08",
    "surface-toolbar": "#130D09",
  },
  extensions: {
    "pulse-before-bg": "#0D0907",
    "pulse-card-bg": "#1E1612",
    "pulse-card-shadow": "0 4px 14px rgba(9,5,3,0.45), 0 1px 3px rgba(9,5,3,0.30)",
    "pulse-control-hover-bg": "rgba(180,140,120,0.05)",
    "pulse-empty-bg": "#241C17",
    "pulse-missed-bg": "rgba(184,92,82,0.18)",
    "pulse-range-bg": "#16110D",
    "pulse-ring-offset": "#1E1612",
    "pulse-skeleton-gradient": "linear-gradient(90deg, #2A1E18 25%, #342820 50%, #2A1E18 75%)",
    "dock-bg": "#110B08",
    "dock-shadow": "0 -1px 4px rgba(9,5,3,0.30)",
    "settings-dialog-bg": "#1E1612",
    "settings-card-bg": "#2E221B",
    "settings-list-item-bg": "#2E221B",
    "settings-header-bg": "rgba(17,11,8,0.60)",
    "settings-sidebar-bg": "rgba(17,11,8,0.50)",
    "settings-nav-active-bg": "rgba(207,160,67,0.14)",
    "settings-nav-hover-bg": "rgba(180,140,120,0.08)",
    "settings-search-bg": "#0E0A08",
    "sidebar-action-hover-bg": "rgba(78,154,83,0.08)",
    "sidebar-active-bg": "rgba(180,140,120,0.06)",
    "sidebar-active-shadow": "inset 0 1px 0 0 rgba(220,190,170,0.06)",
    "sidebar-hover-bg": "rgba(180,140,120,0.02)",
    "toolbar-agent-hover-bg": "rgba(180,140,120,0.08)",
    "toolbar-control-hover-bg": "rgba(180,140,120,0.10)",
    "toolbar-divider": "rgba(54,36,26,0.5)",
    "toolbar-project-bg": "rgba(180,140,120,0.06)",
    "toolbar-project-border": "rgba(54,36,26,0.5)",
    "toolbar-project-chip-bg": "rgba(180,140,120,0.06)",
    "toolbar-project-chip-border": "rgba(54,36,26,0.5)",
    "toolbar-project-shadow": "none",
    "toolbar-stats-bg": "rgba(180,140,120,0.06)",
    "toolbar-stats-border": "rgba(54,36,26,0.5)",
    "toolbar-stats-divider": "rgba(54,36,26,0.5)",
    "toolbar-stats-hover-bg": "rgba(180,140,120,0.10)",
    "worktree-section-hover-bg": "rgba(180,140,120,0.04)",
  },
};
