...
function writeFakeAgent(agentId: "claude" | "codex"): void {
  ...
  writeFileSync(
    ...
    [
      ...
      `  'FAKE_${label}_READY pid=' + process.pid,`,
      `  'FAKE_${label}_COLOR=' + (process.env.DAINTREE_E2E_AGENT_COLOR || ''),`,
      `  'FAKE_${label}_PROVIDER=' + (process.env.DAINTREE_E2E_PROVIDER || ''),`,
      `  'FAKE_${label}_MANUAL=' + manualRun,`,
      "].join('\\n') + '\\n> ');",
