...
| `daintree-assistant` | nothing | Runs in the project root and reads nothing from cwd; it will read the source folders natively when its own skill loader grows user paths. It receives no instructions, reference files, MCP servers or hooks either. |
