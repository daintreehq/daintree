...
**Canopy is a habitat for your AI coding agents.**
...
**The Metaphor:** If VS Code is the workbench where you craft the part, Canopy is the habitat where your agents live, work, and collaborate — and where you observe and guide them.

**Canopy is NOT an IDE (like VS Code). It is NOT a Chat UI (like ChatGPT). It is a terminal-based orchestration layer** — agents run in real terminals, and Canopy wraps them with state intelligence, context injection, and multi-agent coordination.
...
7. **Visual Identity & Comfort** — A rich theme system that makes Canopy feel like a native app, reduces eye strain during long sessions, and supports accessibility needs
...
- **Canopy Assistant** — Built-in AI assistant with orchestration-layer context (sees all panels, worktrees, actions)
...
Canopy is a habitat — a place where developers spend hours every day. Features that help users settle into and personalize their environment are not decoration; they're part of the product's core value proposition.

Ask: _"Does this help the user make Canopy feel like **their** space?"_

Theme discovery (random cycling, search, hero previews), project identity (colors, emojis), and environmental comfort (dock sizing, focus dimming, sound preferences) all serve this principle. They make users want to stay in Canopy rather than reach for another tool.
...
**Engagement and delight are welcome.** Features like streak animations, milestone celebrations, and playful moments make Canopy a place users _want_ to be. The pulse streak is already gamification — and that's fine. Sticky features that reward consistent use reinforce the habitat. The line is features that _only_ exist to manipulate behavior with no underlying value.

## Polish Is Always Welcome

This guide gates **new features** — it does not gate **polish**. Improving an empty state, refining a micro-interaction, fixing a visual inconsistency, or smoothing a rough edge in an existing feature is always accepted work. Canopy's ambition is to be the most polished application in its category. Every interaction should feel considered and intentional.

Polish work does not need to pass the Green Light test. If something already exists in Canopy and it can be made better, smoother, or more delightful — do it.
...
| **Reinforces Identity**           | Strengthens Canopy's visual distinctiveness, user comfort, accessibility, or helps users personalize their habitat |
...
| **Duplicates Agents Without Context** | If the CLI agent already does it well and you're just rebuilding the same thing in a GUI, don't. But wrapping agent capabilities with Canopy's orchestration context (seeing all panels, worktrees, project state) adds real value — that's what the Assistant does. |
...
| Theme System               | **APPROVE**              | Reinforces identity, serves comfort/accessibility, differentiates Canopy. 14 built-in themes with accessibility modes. |
...
Canopy targets all three desktop platforms, but not equally:
...
Canopy is built by a solo developer augmented by AI coding agents — the same agents Canopy orchestrates. This means:
...
- Themes, visual identity, and polish are not cruft — they're what make Canopy feel like a real product instead of a web page.
