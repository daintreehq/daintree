...
| Markdown editing (built-in plugin) | **APPROVE (bounded)** | Plans, specs, checklists and drafts are the working documents of an agent workflow — prose artefacts, not code. A plain-text Markdown editor in the file panel, shipped as a built-in plugin with a revision-checked save and draft recovery, is the smallest thing that removes the round trip to another app. The Markdown allowlist is the boundary: Source stays read-only, code editing stays rejected, and widening to other file types is a separate decision with its own issue (#12323). |
| Visual editing of SvelteKit source (built-in plugin) | **APPROVE (bounded)** | A website is the one artefact where the rendered thing _is_ the spec, and pointing at it beats describing it. The SvelteKit Site Builder may write source, and only through four deterministic, source-backed operations applied to a node the host resolved itself: literal text, literal attribute, literal prop at a verified invocation, and whole Tailwind class tokens. Everything else — structure, behaviour, dynamic expressions, anything the parser cannot prove — is an agent task or a disabled control that explains itself. **The boundary is the operation set, not the file type.** This is not a `.svelte` code editor, it does not widen the built-in-only `fileEditors` allowlist (#12323 still owns that question), and Source stays read-only. The Red Light "reinvents the code editor" still stands for everything outside those four operations. |
...
1. **Higher throughput than traditional solo dev** — But maintenance burden is still real. Every feature must be maintained across 3 platforms and kept compatible with 17 agent CLIs that each update independently.
...
4. **Opinionated integrations** — Support the CLIs that matter (Claude, OpenCode, Aider, Gemini, Antigravity, Codex, Grok, Cursor, GitHub Copilot, Goose, Amp, Crush, Qwen, Kimi, Open Interpreter, Mistral Vibe, Kiro) and support them _deeply_ via the agent registry (`shared/config/agentIds.ts` + `shared/config/agents/`). Don't try to support every AI tool generically.
