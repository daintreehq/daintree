...
| **Voice VAD** | `electron/services/voice/openaiVadWorker.ts`, managed by `electron/services/voice/openaiVadProcess.ts` | `daintree-voice-vad` (**one per voice session**) | one per active OpenAI dictation session | ONNX Runtime can abort in native code (#12577); out of process an abort ends the VAD, not main. Also keeps ~32ms inference off the main loop. See [voice-input.md](../voice-input.md) |

The four long-lived classes — the three hosts and the plugin worker — are forked from a `*-bootstrap.js` (`pty-host-bootstrap.ts`, `workspace-host-bootstrap.ts`, `watchdog-host-bootstrap.ts`, `plugin-dev-worker-bootstrap.ts`) with `stdio: "pipe"` (not `"inherit"` — fd 2 may point at a dead pty on AppImage GUI launches, #5588). The three hosts fork with `cwd: os.homedir()` and tag themselves with `DAINTREE_UTILITY_PROCESS_KIND` in `env`; a plugin worker instead forks at its plugin's own directory. The VAD child is the exception on every count: no bootstrap, forked directly at its compiled entry, `stdio: "ignore"` and no kind tag, because it is short-lived, prints nothing, and speaks only its own message protocol.
...
`brokerPort(host, webContents, { force? })` mints one pair **per webContents (view)**: `port1` → the project's workspace host via `host.attachWorktreePort(port1)` (`postMessage({ type: "attach-worktree-port" }, [port1])`); `port2` → the renderer via `webContents.postMessage("worktree-port", { token }, [port2])`, where `token` is a monotonic per-broker counter. No routing, no fallback — "the port IS the isolation boundary." The token exists because a successful `postMessage` is **not** delivery (#12576): a port posted before the preload listener exists, or into a document that an in-flight navigation replaces, is dropped silently on both sides. The preload attaches the port and then sends the token back on `worktree:port-ack`, and `confirmPort` accepts it only for the view's _current_ token — a receipt for a port that has since been replaced says nothing about the one the view holds now. So the broker **reuses only a confirmed entry to the same host**; an unconfirmed one is replaced by a fresh pair, and `{ force: true }` replaces even a confirmed one, for explicit recovery where the renderer must re-run its ready callbacks and refetch state (the worktree retry handler in `electron/ipc/handlers/worktree/lifecycle.ts`, which then awaits `waitForConfirmation` for 10 s so a failed reconnect keeps the error banner rather than clearing it over a portless sidebar). The broker keys ports by `webContents.id`, holds a reverse `projectPath → Set<wcId>` map, and auto-closes a view's port on `destroyed`, `did-start-navigation` (cross-document main frame), `did-navigate`, or `port1.on("close")`. `did-navigate` covers what `did-start-navigation` cannot: a navigation already in flight when the port was posted fired its start event before the entry existed, so the entry is retired at the commit and the new document's `did-finish-load` re-broker posts one it will actually hold. On host restart, `closePortsForHost(projectPath)` snapshots the view IDs, closes them, and `reBrokerForHost()` re-creates them against the fresh host (wired in `electron/window/windowServices.ts` on the `host-restarted` event).
...
- **`electron/window/ProjectViewManager.ts`** — per-project `WebContentsView` lifecycle. Each project gets its own `WebContentsView` with an **independent V8 context** (Site Isolation). `switchTo()` swaps the visible view (a cached view returns in <16 ms). View state is `"loading" | "active" | "cached"`. Cached views are hidden, told they are cached so the renderer demotes its own periodic work, and, while the Efficiency profile is on, frozen (`Page.setWebLifecycleState`) unless a live agent or MCP activity protects them. They are never CPU-throttled: `Emulation.setCPUThrottlingRate` is a slow-device simulation that busy-spins the renderer main thread, frozen or not (#12456). A paint gate keeps the outgoing view on screen until the incoming one paints (soft 1500 ms / hard 4000 ms) so the user never sees an unpainted frame.
- **LRU eviction** — `evictStaleViews(reason)` (`"lru" | "pressure" | "limit-change"`) destroys the least-recently-used views once `views.size` exceeds `maxCachedViews`. That cap resolves in `effectiveCachedProjectViews()` (`electron/utils/cachedProjectViews.ts`) in a fixed precedence: a stored user setting wins if `isValidCachedProjectViews` accepts it (an integer in 1–5 — invalid values are ignored, not clamped), then E2E mode pins it to 4, then `computeDefaultCachedViews()` picks it from installed RAM — **2** below 16 GiB, **3** at 16–32, **4** at 32–64, **5** at 64 and up. **5 is the hard ceiling in every path**, and the cap counts the active view, so the most background workspaces that can stay warm is four. Under memory pressure `memoryPressureTarget()` steps the effective cap down one view per slice of the warning→critical band and collapses it to 1 below critical, **per pass** (`maxCachedViews` is never mutated). Eviction is **pure LRU, not size-first** (#8602): the biggest renderer is usually the project you're working in, so size-first would evict the most valuable view. A workspace the user has pinned via **keep-resident** (`workspaceKeepResident`, #12313) is ordered last in the candidate queue rather than removed from it, so it is surrendered only once every ungranted candidate is gone — a grant, not a floor: it never carries the cache over the configured cap, and a critical reclaim that collapses the effective cap to 1 takes the grants too. Only the user can set it, through project settings. On eviction the view's port is cleaned up via the registered eviction callback.
