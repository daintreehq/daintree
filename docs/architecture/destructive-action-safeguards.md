...
| `terminal.arm` (#10695) | **confirm** (updated #10695) | none — agent/MCP-only, `palette: { mode: "hidden" }`, no user-facing dispatch (user arming goes through the fleet ribbon `toggleId`/`armAll`) | gated on `confirmed:true` for agent dispatch | reversible (`terminal.disarm`/`terminal.disarmAll`) | armed set (arming reroutes the human's keystrokes to every armed terminal) | D0 (user) → D1 (agent-dispatched) | Done (#10695) — exposed over MCP so external clients can manage the broadcast set; on `EXPECTED_CONFIRM_DANGER` + `BYPASS_WIRED` (agent-dispatch gate, no co-located UI dialog) | — |
