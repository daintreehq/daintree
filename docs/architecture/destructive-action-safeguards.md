...
| `scratch.deleteAll` (Scratch section header context menu) | **confirm** (added #11086) | yes — `ConfirmDialog` in `ProjectSwitcherPalette.tsx` (state in `useProjectSwitcherPalette.ts`); count-only title off a snapshot frozen at confirm-open, no typed-name gate (D1, matching `worktree.bulk.closeSessions`) | n/a (direct-IPC `window.electron.scratch.remove` via `scratchClient` → `scratchStore.removeScratch`, ActionService bypass — confirm wired in component) | local-irreversible (terminals killed, panels cleared, DB row tombstoned, folder `fs.rm`'d) | every scratch workspace | D1 | Done (#11086) — confirm wired in component; fan-out via `Promise.allSettled` with one summary notification | #11086 |
