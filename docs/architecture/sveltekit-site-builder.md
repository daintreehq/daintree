...
- **Button** in the preview's browser toolbar. The host shows it only where the tool's `isAvailable` predicate says yes — the Site Builder's asks `detect-apps` whether the worktree holds a SvelteKit app (cached per worktree, a "no" never reused) — so other sites never show it, and `devPreview.toggleTool` refuses there with the tool's own `unavailableReason`.
