...
The workspace package is the repo's own established mechanism, already used by five packages. Version skew is then handled where it belongs — in the **support verdict**. The project model reads the project's installed `svelte` and `@sveltejs/kit` versions and reports `preview-only` with a specific reason when either is outside the supported major — below it or above it, since a newer major inherits nothing it was tested against — rather than parsing with a mismatched compiler and hoping. `preview-only` gates direct edits only — clicks are still traced to source, with every surface inspect-only, and the selection can still go to an agent. Tailwind does not decide direct editing: class edits write tokens exactly as typed. Class awareness (suggestions, inspection, conflicts) is reported separately by the `tailwind-status` channel, which distinguishes a site that doesn't use Tailwind (nothing to show) from one whose Tailwind couldn't be loaded (a reason in the **Site source** section). The drawer renders the two as separate capability rows from that structured data, never by matching reason text.
...
The same rule applies to Tailwind, and was nearly missed. The design system is compiled by **Daintree's own bundled `tailwindcss`**, never the project's copy: the project's package is read only as data — its version for the support verdict, its stylesheets for the theme. Modules a project's CSS names through `@plugin` or `@config` are **not loaded**; they resolve to inert stand-ins and are listed in the load result's `skippedModules`. The cost is that utilities a project adds through a JavaScript plugin are missing from completion, and the model says so: a class inspected against a model with skipped modules is qualified rather than called empty. The alternative was executing any opened repository's JavaScript inside Electron main the moment the inspector read its CSS.

Project CSS decides what gets read, so reading it is bounded: every stylesheet must resolve inside the worktree or be a dependency's own stylesheet under `node_modules`, each file is capped at 2 MB, everything read at 16 MB, and imports at 256 files besides the entry, with the count and byte budget reserved before each read so imports loaded side by side can't all pass the check. The compiled system is cached per workspace and rebuilt when the entry or any of the project's own imported stylesheets changes. For imports the revision is the hash of the bytes that were compiled, and an import symlink retargeted to another file counts as a change; the entry is compared by a read taken just before compiling, which a change in between corrects on the next request.
...
## Separate facts, never one green tick

The single most common way a visual editor lies is collapsing distinct facts into one green tick. The receipt keeps them apart:

| Fact | What the receipt says today |
| --- | --- |
| Source saved | Proven: the write returned, with the new content hash, and what changed. |
| Preview refreshed | "reloaded" once the guest reports a later document; "refresh unconfirmed" otherwise. A hot update inside the same document isn't counted as a reload. |
| Styles generated | Always "unverified": nothing reads computed style back yet. A class reaching the DOM does **not** mean Tailwind emitted a rule for it. |

Reading computed style back and a user-marked "reviewed" state are the intended next evidence, not current behaviour.
...
- **Drawer** beside the page, open only once there is something to show: selection identity, **Edit directly** and **Ask an agent** as peer sections (editing first), and the last change with Undo. It is 360px where the panel has room and gives way down to 280px in a narrow tiled panel. When the worktree holds more than one app, **Site source** keeps an app switcher; switching drops the selection, receipt and Undo, which belonged to the other app.
...
- **Text and classes** are the direct surfaces. A class is replaced, removed or added as one write and one Undo: **Replace…** on a chip swaps it, and adding a class that conflicts with one already there (the same box slots in the same variant scope, from `tailwind-conflicts`) asks **Replace** or **Keep both** instead of leaving two competing tokens.
- **Inspecting a class compiles that exact token** (`tailwind-describe`). The suggestion list is a bounded search that omits variants (`hover:p-4`) and arbitrary values (`w-[37px]`); it is never used to say a class does nothing.
- **A surface the builder won't edit still says how it is written.** A dynamic class list, a class directive, a spread or an expression in the text shows its source (`written` on the selected node, read off the compiler's AST) beside the reason it needs an agent: each class-bearing attribute on its own, whitespace kept, bounded in length. It is shown in the drawer and not sent to agents.
- **With a component picked, direct edits change its root element**, and the section says so with the root's own file and line; a component with several roots explains that edits are single-element.
- **The receipt says what changed** — `px-6 → px-8`, `+ shadow-md`, or the text before and after with line breaks kept visible — and reads the other way after Undo. Saved, reloaded and styled stay separate facts.
- **Editing continues through a write.** The page is asked for the same element again — the same rendered occurrence, and the same component when one was picked — and main re-proves it at the revision just written. A copy the page couldn't place (past its 20,000-element scan) is never guessed; the selection goes stale and asks to be picked again. Paths in that proof are compared worktree-relative, so it holds in monorepo apps.
