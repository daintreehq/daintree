...
The workspace package is the repo's own established mechanism, already used by five packages. Version skew is then reported where it belongs — in the **support verdict**. The project model reads the project's installed `svelte` and `@sveltejs/kit` versions and answers `untested`, with a reason naming the package and version found, when either is outside the major the bundled compiler was tested against: below it, or above it, since a newer major inherits nothing this was tested against. It also answers `untested` when it cannot establish the versions at all — nothing installed, a manifest it could not read, or a Yarn Plug'n'Play tree whose resolution it refuses to guess at. `tested` is the narrow case: both packages read, both at the tested major. Tailwind is not part of the verdict; its version is read for the agent's context and nothing else.

The verdict is a **diagnostic, not a gate**. Parsing does not branch on it: `main/selection.ts` always resolves through the bundled compiler (`main/engine.ts`), so an `untested` app is traced and handed to an agent exactly as a tested one is — which is why the vocabulary is `tested`/`untested` and not the `full`/`preview-only` of the capability split the builder no longer has. What it buys is the ability to say what the compiler that traced a range was tested against, and it says that in the two places that can weigh it: the drawer's **Site source** section carries a neutral notice for `untested` (an observation, `info` tone, no accent and no recovery action — nothing is worse for it, so it never opens the drawer by itself), and the agent prompt gains one line telling the agent to verify the source before relying on the location. The prompt's line is derived from the installed versions it already lists, not from the verdict — so it names a version read from disk and claims nothing about what the app resolves. Nothing else reads the verdict.
...
**The importer owns the dependency.** `svelte/compiler` is reached only through the package's own `loadParse()` (`src/parse.ts`), never from the plugin directly. The plugin used to import it itself, from outside the workspace that declares it, which resolved only because npm hoists — a change to the install layout, or a second Svelte version anywhere in the tree, could have changed which compiler it got or failed outright. `svelte/compiler` now appears in exactly one file in the repo outside tests, so the barrel cannot drag it onto a startup path either. `scripts/lazy-compiler-boundary.test.ts` builds the plugin's real main entry with esbuild and walks the metafile: the compiler and the source model must be in the bundle and must not be in the eager graph, so the test fails both if the boundary breaks and if it becomes vacuous.
...
**Validation happens twice, and the halves own different things.** The envelope is the host's: `electron/services/sitePreview/guestProtocol.ts` checks those five fields, requires a `type` on the event, and validates exactly one payload — `documentReady`, because the bridge acts on it to mark a binding ready. Every other event crosses to the renderer as an opaque body the host never read. The payload is the adapter's: `GuestEventSchema` in `plugins/builtin/sveltekit-builder/shared/protocol.ts` is the full Svelte-shaped union, and `renderer/inspectorController.ts` parses each event against it before the switch, dropping what fails with one bounded warning. Core used to mirror that union — a `no-svelte-meta` issue code and a Svelte ancestry chain in `electron/` and `shared/` — which meant every new framework adapter would have had to add a member to core for a payload core does not use. `SitePreviewBridgeProtocolDrift.test.ts` now pins only what the two halves still share: the envelope's field set, the `documentReady` shape, the protocol version, the message ceiling and the declared adapter id.

**The runtime goes only where the adapter said.** A preview can navigate anywhere — the app links out, an OAuth flow leaves for a provider and comes back — and "this panel started on localhost" is not a lifetime rule. Every adapter carries an origin policy (`electron/services/sitePreview/originPolicy.ts`; `local-preview` by default: loopback, `*.localhost`, `*.local`, RFC 1918), and the bridge checks the guest's URL on every install, at bind and after each committed navigation. On a document outside the policy it suspends rather than detaches: the new-document script has already run in that document by the time the navigation is observed, so the script is removed, the CDP binding is removed (which is what closes the tap), the runtime is told to dispose itself as on teardown, and traffic from that document is ignored. The binding survives, the renderer is told with an `origin-policy` push, and the next document back inside the policy installs without a rebind. The strip shows the pause as an observation with no action.

**The script refuses itself.** The origin policy above is checked host-side, but the host cannot check it in time: `Page.addScriptToEvaluateOnNewDocument` has already run in the new document by the moment main observes the navigation, so suspension is a removal after the fact and cannot un-intercept an event. The policy therefore travels _with_ the script — the prelude's first statement is the policy evaluated against `location.href`, and a denied document gets no global, no listeners and no adapter body. It is the host's own predicate serialised with `toString()` rather than a copy of the rules in a string literal, so the page's verdict and the host's cannot drift; `originPolicy.test.ts` evaluates both over the same URLs to prove it, and `guest-host-integration.test.ts` runs the real prelude wrapping the real guest asset at `https://example.com/oauth` and asserts nothing installs and no click is intercepted.

**A disabled plugin's runtime does not run.** Adapters are all registered at startup, which is a fact about packaging and says nothing about lifecycle, so `bind` asks whether the owning plugin is loaded before it reads the adapter's body — and again after, because the read is an `await` a disable can land under. That covers the next document; the one already on screen is covered from the other side, by an unload step in `PluginService` that tears down every binding the unloading plugin owns (`owner-disabled`). Without it a binding made while the plugin was enabled would keep reading the site and pushing observations at a renderer whose plugin is gone, until the user happened to navigate.
...
An earlier version of this plugin did write — planned replacements over ranges of the original source, an `expectedRevision` on every write, an in-memory journal for Undo, and a re-proof of the selection after each one. The drawer never called it (see below), so it is gone: the engine, the journal, the undo channel, the receipt, and the Tailwind class model that existed to validate what those edits wrote. `packages/svelte-source-model`'s edit planner outlived it by one round — kept on the theory that a future caller might want it — and has now gone too, along with the splice buffer and the class-token and entity helpers that existed only to serve it. Nothing imported any of it but its own tests. The package is a resolver now: give it bytes and a dev-runtime location, and it tells you which element that is.
