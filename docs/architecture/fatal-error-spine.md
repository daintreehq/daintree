...
2. **`cleanupOnExit()` runs only on the success branch of the cleanup chain.** In `shutdown.ts`, the marker is removed inside the `Promise.race` success handler, before the safety-belt timer fires. The error/timeout branch deliberately does **not** call `cleanupOnExit()` — leaving the marker on disk is the dirty-exit signal.
