...
The lowest tier. A grid panel's border colour encodes its agent state — no `notify()` call, no history, no dismissal. `ContentPanel` selects one of these classes in a ternary (`src/components/Panel/ContentPanel.tsx`):

- `panel-state-waiting` — agent waiting for input (`--color-activity-waiting`)
- `panel-state-working` — agent working (static border; the old 4s breathe was removed as a WCAG-COGA fatigue trigger, see CSS comment at `src/index.css`)
...
The CSS, the edge geometry vars, and the dedicated `forced-colors` / `prefers-contrast` overrides all live in `src/index.css`. These borders coexist with the voice-lock border by design — a panel can be both working and dictation-locked.
...
`NotifyPayload` (`src/lib/notify.ts`) is a discriminated union on the message shape:
...
This is the load-bearing invariant: a rich toast cannot exist without a plain-text inbox fallback, so the durable WCAG-conforming row is never silently dropped. `CoalesceOptions` (`src/lib/notify.ts`) mirrors the same union for the coalesce patch path so a `buildMessage` returning a ReactNode is forced to supply `buildInboxMessage`.

### Priority routing matrix

`priority` is `"high" | "low" | "watch"` (`src/store/notificationStore.ts`). The routing matrix (documented at `src/lib/notify.ts`):
...
Rather than every call site re-running the four-question checklist, a kind-based manifest fills routing defaults. `resolveEventPolicyDefaults()` (`src/lib/notify.ts`) runs first in `notify()` and fills only the gaps the caller left — explicit fields always win. The taxonomy `NotificationEventKind` (`completed`, `waiting`, `workingPulse`, `uiFeedback`, `agent`, `git`, `host`, `recovery`, `settings`, `connectivity` — `src/lib/notify.ts`) is kept a _closed_ union for compile-time completeness on `EVENT_POLICY` and `EVENT_KIND_LABEL`. Each kind declares a `baseInterruption` (`passive | active | time-sensitive | critical`) mapped to a priority by `INTERRUPTION_TO_PRIORITY` (`src/lib/notify.ts`: `passive→low`, `active/time-sensitive→high`, `critical→watch`), an optional `defaultDurationMs`, and an optional `userOverrideKey` (the persisted silence toggle). `EVENT_KIND_TO_SETTING_KEY` is derived from the manifest so it stays the single source of truth.
...
- **Active-context suppression** (`src/lib/notify.ts`): a focused high-priority notification whose origin surface (matching `context.worktreeId`/`panelId`) is already visible is suppressed and recorded inbox-only. A 500ms grace (`SUPPRESS_GRACE_MS`) catches navigate-away and window-blur races — if the user leaves before it expires, the suppressed event is promoted to a real toast. Accessors are wired at boot in `src/store/rendererStoreOrchestrator.ts` via `setActiveContextAccessors`.
- **Transient-error escalation** (`src/lib/notify.ts`): auto-retryable errors (EBUSY/EAGAIN/etc.) route to `low` by default; when the same normalized error repeats past a threshold within a window (local-resource 3/5s, network 3/120s) the next instance escalates to a toast, one-shot per group with a 60-min cooldown.
- **Per-source token-bucket rate-limit** (`src/lib/notify.ts`): catches slow-dripping noisy producers outside the coalesce window. 3 tokens, refill 1/10s; on overflow the would-be toast is dropped and an in-place `priority: "low"` summary inbox row ("{N} more events") tracks the count. Bypassed for `priority:"low"`, `transient`, `placement:"grid-bar"`, `urgent`, and `coalesce` (each is its own gate or has no inbox fallback).
- **Coalesce** (`src/lib/notify.ts`): same `coalesce.key` within ~2s collapses into a single updating toast with a count badge.
...
`TOAST_DURATION` (`src/lib/notify.ts`) sets per-type auto-dismiss (error/warning 8s, info 6s, success 5s); action-bearing toasts default to sticky (`duration: 0`) so the action stays reachable.
...
Persist v1→v2 (`migrate` at `src/store/slices/notificationHistorySlice.ts`) adds the `snoozedThreads` map (issue #9200); `merge` re-sanitizes every entry off the wire (`sanitizePersistedEntry` forces `message` to a string and rejects implausible timestamps) so a corrupt blob can't crash React on render.
...
**OS-DND is display-only and a sound gate — never a toast suppressor.** `OsDndService` probes macOS Focus via `~/Library/DoNotDisturb/DB/Assertions.json` and the private `_NSDoNotDisturbEnabled/Disabled` Darwin notifications; Windows/Linux are left `undefined` (fail-soft). It is consumed by the informational-audio gate in `AgentNotificationService` (working pulse and all-clear) and by the read-only DND indicators in the renderer (toolbar button + Notification Center). The OS already silences its own native banners, so gating in-app toasts on it would double-suppress (`src/store/notificationSettingsStore.ts`, `electron/services/OsDndService.ts` class JSDoc).

Session mute crosses the process boundary: `setSessionQuietUntil` (`src/lib/notify.ts`) writes the module-level `_quietUntil`, mirrors to `useNotificationSettingsStore` for the toolbar bell, and calls `window.electron.notification.setSessionMuteUntil` so main-side completion notifications and working-pulse sounds honour the same window.
...
- Entry point: `src/lib/notify.ts` — read `notify()` top-to-bottom for the full routing order.
- Payload contract: `NotifyPayload` (`src/lib/notify.ts`) and the `CoalesceOptions` mirror.
