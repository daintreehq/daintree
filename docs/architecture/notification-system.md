...
**OS-DND is display-only and a sound gate — never a toast suppressor.** `OsDndService` probes macOS Focus via `~/Library/DoNotDisturb/DB/Assertions.json` and the private `_NSDoNotDisturbEnabled/Disabled` Darwin notifications; Windows/Linux are left `undefined` (fail-soft). It is consumed in exactly two places: the informational-audio gate in `AgentNotificationService` (working pulse and all-clear) and the read-only toolbar tooltip. The OS already silences its own native banners, so gating in-app toasts on it would double-suppress (`src/store/notificationSettingsStore.ts`, `electron/services/OsDndService.ts` class JSDoc).
