...
### Design constraints for the remaining fronts (recorded, not yet built)

- **Remote surfaces break the sync half of `TerminalPaintPlane`.** The plane interface has synchronous value-returning methods (`get`, `captureBufferText`, `getAgentState`, `isFocused`, `fit`, `resize`) that cannot cross a process boundary. A `WebContentsView`-hosted surface therefore needs either an interface split (sync-local vs async-remote-safe subsets) or compositor-side state mirrors for the sync reads. This is the load-bearing design decision for view hosting.
- **`attach(id, container)` cannot cross views.** Cross-view surfaces replace DOM attachment with a bounds protocol (window-manager positioning of the surface view over grid pane rects). Electron has no per-pixel input pass-through for stacked transparent views, so surface views must own rectangular regions — placement becomes geometry-constrained (band-aligned), which composes with (and constrains) the atlas/hot-cold policies.
