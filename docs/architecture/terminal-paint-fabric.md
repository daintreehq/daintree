...
Phase 0 is landed behind `DAINTREE_PAINT_FABRIC` (default off). The Phase 1 compositor semantics are landed (see "What is landed" below): the watch-list is resolved, the cross-surface transfer protocol, the crash fallback (retire/co-locate — the kill-switch behavior every phase requires), and naive placement (round-robin) all exist behind the flag with deterministic tests. `DAINTREE_PAINT_FABRIC_SURFACES=K` (clamped, default 1) builds K in-process surfaces to exercise the multi-surface semantics live. The Phase 2/4/5 machinery (atlas clustering, aggregate WebGL budget waterfill, worker-parse subsystem, bounded-K scheduler) is built and tested on the follow-on branch. The two remaining integration fronts — hosting surfaces in sibling `WebContentsView`s (#10959) and wiring worker-parse into the live ack-coupled ingest path (#10960) — are scoped follow-ups; their design constraints are recorded below so they land against decisions, not discoveries.
...
- **Placement (Phase 1).** `placementPolicies.ts`: round-robin (the naive Phase 1 policy) and least-loaded (the retire fallback's sibling choice). GPU-mode isolation is proven at this layer: 13 WebGL-promoted terminals flag-off flip the whole fleet to DOM; flag-on with placement spreading, no surface crosses the threshold and the flip never fires. In-process aux surfaces are forced to DOM rendering at construction so K>1 cannot multiply-pack the window's shared 16-context budget.
