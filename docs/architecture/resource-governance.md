...
## Terminal retention tiers (lossless memory diet)

Since terminal hibernation was removed (#10811), every background terminal stays fully live — so the pty-host's steady-state memory is dominated by per-terminal analysis structures: the headless mirror scrollback (`scrollbackLines × cols × 12` bytes each), preserved exit snapshots (~1–4 MB each), and the smaller semantic/forensics/agent-output tails. A retention policy makes those explicitly budgeted and state-aware instead of incidentally sized.

Three cooperating pieces:

- **Policy** — `shared/config/terminalRetention.ts`. `computeRetentionTier()` maps observable terminal state to one of four tiers: `foreground` (focused, or visible working agent, or visible plain shell — full budgets, identical to the historical fixed caps), `working` (background working/directing agent — mirror capped at 5000 lines, matching the renderer's agent scrollback ceiling so nothing user-visible is lost across a view-eviction restore), `settled` (waiting/idle/completed/exited — mirror 2000 lines, matching the renderer's plain-terminal ceiling), and `archived` (trashed or preserved-snapshot terminals — minimal budgets, preserved snapshots LRU-pruned). Each tier also carries a `pressureMirrorFloorLines` — the floor the governor's targeted trim may take that tier down to.
- **Coordinator** — `TerminalRetentionCoordinator` (`electron/services/pty/TerminalRetentionCoordinator.ts`), a 5 s sweep in the pty-host. It stamps each terminal's tier and applies budgets via `TerminalProcess.applyRetentionTier()` (semantic/forensics/agent-output caps plus mirror trim/grow). Every terminal is born stamped `foreground` (a pure stamp — foreground budgets equal the construction-time caps), so the governor never sees an untiered, unprotected terminal in the gap before the first sweep. `applyRetentionTier` no-ops on an unchanged tier, so a pressure trim below the tier cap is never undone until a genuine tier transition (e.g. a settled agent starts working again).
- **Governor integration** — `getTerminalBufferSizes()` entries carry the stamped tier, so the pre-pause targeted trim is state-aware: preserved snapshots are pruned first (pure retained memory, `PRESSURE_MAX_PRESERVED_TERMINAL_SNAPSHOTS = 5` vs the steady-state cap of 20), then buffers trim heaviest-first to their tier floors, with `foreground` terminals exempt. Only the critical-pressure path (≥95% raw) touches foreground buffers — and it does so AFTER the immediate global pause, as an emergency floor-trim so utilization can actually recede before the 10 s force-resume.

Invariants the policy holds by construction: the renderer xterm is never touched (it remains the visual source of truth — a user scrolling back sees the renderer's retained scrollback); live PTY output is never suspended or dropped for memory savings; trims are recorded per terminal as **memory retention trimming** (`trimCount` / `lastTrimAt` / `lastTrimReason`) in the flow-control snapshot, distinct from `droppedBytes` (actual output data loss). Two staleness guards keep trims from corrupting snapshots: every destructive mirror trim bumps a per-terminal `trimEpoch`, and `SessionSnapshotter` discards any serialize whose await overlapped a trim (the next flush persists the clean post-trim state); in worker mode, `set-scrollback` messages carry the slot generation so a stale trim from a superseded backend can't shrink a rebuilt session's mirror.

`get-flow-control-snapshot` surfaces the whole picture per terminal: retention tier, per-category retained-byte estimates (mirror / semantic / forensics / agent-output / preserved snapshot), and trim history — so a support bundle can attribute which terminals and which buffers dominate host memory.
