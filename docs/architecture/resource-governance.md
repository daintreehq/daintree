...
When the lag latch engages it calls `applyProfile("efficiency")` directly, **bypassing `computeTargetProfile()` and all hysteresis** — saturation needs an immediate reaction. Because that path skips the memory measurement, it first zeros `lastMemoryScore` so the efficiency entry does **not** trigger the destructive `setCachedViewLimit(1)` clamp (the trigger here is CPU, not memory). While the latch is held at `efficiency`, `evaluate()` refuses to upgrade out of it; recovery is owned solely by the lag exit path.
...
Engage is gated by warm-up (5 ticks) and a 30 s re-engage cooldown after any force-resume (prevents the pause/resume flap of #8616). **Critical pressure (≥95% raw) bypasses both** — the next allocation could OOM the host. Before pausing, the governor attempts a one-shot `trimBuffers()` reclaim (`:204`); if pressure persists a tick later it pauses. Pause/resume order is triaged: idle terminals pause first and active agents pause last (and resume in the reverse order) so working agents get maximum runway. Paused terminals emit the `paused-resource-governor` flow status; on resume they revert to `running` (or restore `paused-backpressure` if the backpressure token is still held). See [terminal-lifecycle.md](./terminal-lifecycle.md) for the full flow-status state set and pause-token coordination.
