...
- **The result envelope says which surface answered.** `source` is `"renderer"` or `"pty"`, and `unavailableFields` lists what that surface could not observe (`["armed", "lastCheckResult"]` for `pty`, empty for `renderer`). Absence alone would read as "not armed", which main has no evidence for.
...
- **Fallback, not replacement, and only for `not-found`.** An `ambiguous` binding is a conflict the user must resolve and a destroyed pin is not a route that comes back, so neither takes the fallback. Records outside the bound workspace, dev-preview PTYs and the assistant terminal report as `Terminal not found` — a bound session must not be able to confirm an id it has no route to.
