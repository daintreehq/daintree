...
None of the `*Owned` tools is in any of this. Their ownership checks are main-side, but each one delegates to a renderer action — `terminal.close` for trash and recovery, `worktree.delete` for its D2 preview, `pilot.openRun` for the switch — and that delegation is renderer work.
...
The generic `terminal.close` and `worktree.delete` stay off `MCP_EXTERNAL_TIER_TOOLS`, and so does `pilot.openRun` — it is reachable only through the `terminal.revealOwned` short-circuit. The same goes for `terminal.interrupt` and `fleet.interrupt`: the unscoped interrupts reach any pane a listing names, including the user's own mid-turn. All six `*Owned` tools execute as main-process short-circuits because the ledger is keyed by MCP session id, which the renderer never sees.
