...
`shouldExposeTool` (used by `tools/list`) and `isTierPermitted` (used by `tools/call`) are the two gates, and they enforce different things. `isTierPermitted` owns tier membership and nothing else. `shouldExposeTool` layers the **advertisement-only** filters on top and then defers to `isTierPermitted`, so membership can never drift between the two. Eager listing is an opt-in: an entry is advertised only when it declares `mcpVisibility: "core"` and is not `danger === "restricted"`. Everything else is deferred, which keeps the eager envelope small by construction as the allowlists grow (#11540). Unset and `discoverable` are deferred but still discoverable — found via `actions.search` / `actions.getSchema`; `hidden` is deferred and undiscoverable, dropped from introspection too. Those metadata filters keep a tool out of `tools/list` but do not by themselves block dispatch: a deferred tool that is tier-permitted (e.g. `worktree.resource.teardown`) is deliberately callable once an agent finds it via the meta-tools. `restricted` actions are the exception that really is unreachable — `ActionService.dispatch` rejects them independently of tier.

The core cohort is declared per-action rather than in a central list, so an action's visibility sits beside its schema and description, and `rg 'mcpVisibility: "core"'` is the surface. `actionDefinitions.quality.test.ts` guards it against silent re-bloat: it derives the cohort from the real registry and asserts a 15–25 budget, that every core id is dispatchable at some tier, that none is `restricted`, and that the discovery entry points (`actions.list`, `actions.search`, `actions.getSchema`) stay core — without them the deferred surface is unreachable, since a caller cannot search for the search tool.

The introspection tools (`actions.list`, `actions.search`, `actions.getSchema`) are narrowed by a third gate, `filterIntrospectionResultForSession`, applied in main to the dispatch result (#11525). It layers `isTierPermitted`'s allowlist — widened by any live per-tool or native automation grant — on top of the `hidden`/`restricted` ceilings, so discovery returns what the session can actually call. It deliberately does **not** reuse `shouldExposeTool`: dropping everything that is not `core` there would defeat progressive disclosure, which depends on search and schema lookup surfacing precisely the actions `tools/list` omits — now most of the registry.
...
The `forge.*` tools are how an agent reads and mutates the project's issues, pull requests, and reviews over MCP. Every action is defined in [`src/services/actions/definitions/forgeActions.ts`](../../src/services/actions/definitions/forgeActions.ts); the `run()` bodies stay provider-agnostic and provider routing (GitHub, GitLab, …) is resolved at the IPC layer in `electron/ipc/handlers/forge.ts`. For how a provider is selected and normalized, see [`forge-provider-abstraction.md`](./forge-provider-abstraction.md) — this section documents only the MCP-facing contract (id, args, tier, `danger`, rate limit, dedup). No forge action is `mcpVisibility: "core"`, so none is eagerly listed; every one stays dispatchable at the tiers below and is found through `actions.search` / `actions.getSchema`. A client operating on issues and PRs generally has its own tools for that, which is why the whole surface is deferred rather than spending eager slots (#11540).
