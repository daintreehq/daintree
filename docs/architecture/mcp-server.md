...
| `resourceOwnership.ts` | Which session created which terminal/worktree, written only from trusted post-dispatch results. Backs `terminal.closeOwned` / `worktree.deleteOwned` / `terminal.revealOwned` / `terminal.interruptOwned` / `terminal.sendCommandOwned` / `terminal.injectOwned` / `terminal.readLastMessageOwned` — the reveal reads the recorded `workspaceId` to know where the panel actually lives — and `terminal.list`'s `owned` filter reads it. See [Resource ownership](#resource-ownership-11909). |
...
3. **Per-pane token.** `mcpPaneConfigService.isValidPaneToken(token)` — tokens minted for a specific terminal pane. `getTierForToken` maps the pane token to `core`/`full`, so a pane gets exactly the surface its config grants, never `external`.
...
`resolveTokenTier` mirrors `isAuthorized` and returns the `McpTier` for the bearer: API-key/empty → `external`; pane token → its mapped tier; help token → its help tier; otherwise the `core` baseline.

## Tier model (`shared.ts`)

```ts
type McpTier = "core" | "full" | "external";
```
...
| --- | --- |
| `core` | `CORE_TIER_TOOLS` — the orchestration surface and the default for the in-app assistant: worktree creation and the waits on setup and PRs, agent launches, prompts, terminal reads and waits, interrupt, move, rename and close, deleting a worktree the session created, and action search. |
| `full` | core ∪ `FULL_TIER_ADDONS` — recipes, workflows, project checks, forge and git reads, CopyTree, the unscoped worktree delete and resource lifecycle, kill/restart/close-all, skills, diagnostics, and the plugin-authoring loop. |
...
The `external` surface is deliberately small (#11585) — it stood at 100 entries / ~128 KB of schema, past what MCP clients tolerate, and [Payload and registry budgets](#payload-and-registry-budgets-11526-11585) covers the caps that now bound it. The selection rule is **what only Daintree can do**: terminal and agent orchestration, worktrees, recipes, skills, live IDE context. An external agent driving Daintree over MCP is sitting in a terminal with its own shell and its own `gh`, so git plumbing, forge reads and writes, file reads, and project queries are its job. The in-app tool sets follow the same rule. Before the core/full split they were a three-rung `workbench`/`action`/`system` ladder of 181 tools, most of which no assistant ever called — the retired Daintree Assistant's own dispatch log over two months was almost entirely terminal reads, sends and agent launches — while every one of them was re-sent to the model on each turn. `core` carries that orchestration loop; `full` adds the reads and helpers an orchestrator reaches for less often. Everything else is on **no** tier: git writes and most git reads, forge writes and browser openers, file reads, UI navigation and layout, theme and settings writes, session bookmarks, the recipe editor, fleet arming, bulk kills, and `agentSettings.get` (its result carries launch flags and preset payloads that can hold credentials). Those stay ordinary actions for menus, keybindings and the palette.

Stored settings from before the split are read in place: `workbench` and `action` become `core`, `system` becomes `full` (`normalizeHelpAssistantTier`, `normalizeDaintreeMcpTier`). The old `system` tier also pre-authorized the ordinary confirm for an agent pane (#12692); no tier does that any more, so `full` means more tools and nothing else.

Two consequences worth knowing. `git.push` keeps its bespoke MCP plumbing (branch/commit preview in the confirm dialog, cwd pinning against a mid-modal worktree switch, the headless-safe confirm path), but it is on no tier, so nothing reaches it over MCP. And because MCP resources authorize through their backing action (`RESOURCE_BACKING_ACTIONS`), external sessions no longer read `daintree://project/current/issues` or `daintree://worktree/{id}/pulse`; terminal-scrollback and agent-state resources are unaffected.

`shouldExposeTool` (used by `tools/list`) and `isTierPermitted` (used by `tools/call`) are the two gates. `isTierPermitted` owns tier membership, read through the session's origin (below), and nothing else. `shouldExposeTool` layers two ceilings — `danger === "restricted"` and `mcpVisibility: "hidden"` — on top and then defers to `isTierPermitted`, so membership can never drift between the two. `restricted` actions are unreachable regardless of tier: `ActionService.dispatch` rejects them independently.

**Unscoped panel tools are reserved for renderer-owned origins (#12407).** The ladder tiers are not only the assistant's: a Claude pane launched in a project with Daintree MCP enabled gets its own bearer at the project's tier, with origin `external`. `terminal.sendCommand`, `terminal.inject`, `terminal.close` and `worktree.delete` accept any id a listing returns, so granting them to that session let an agent in a read-only sandbox type into or close a neighbouring shell and run commands as the user. `OWNED_TWIN_TOOLS` pairs each with its owned form, `RENDERER_OWNED_ORIGIN_ONLY_TOOLS` names the unscoped tools with none (`copyTree.injectToTerminal`, `terminal.kill`, `terminal.restart`, `terminal.closeAll`), and `getTierPermittedActionIds(tier, rendererOwnedOrigin)` returns `NON_RENDERER_OWNED_TIER_ALLOWLISTS[tier]` — the same surface with each unscoped tool swapped for its twin and the rest removed (`toNonRendererOwnedTools`) — for every session that is not `help` or `assistant-pane`. The assistant's own sessions are never listed the owned send, inject or close, since the unscoped tool already does that job; `worktree.deleteOwned` is the exception, because it is core while the unscoped delete waits for `full`. Discovery, the call gate, `mcp.surface` and the target policy all read that one selector, and it defaults to the narrower set. Such a session submits text, injects context, closes a terminal or deletes a worktree only through `terminal.sendCommandOwned`, `terminal.injectOwned`, `terminal.closeOwned` and `worktree.deleteOwned` (see [Resource ownership](#resource-ownership-11909)). Two launch arguments are refused alongside it, because either would hand back what the reservation takes away: `agent.launch` under a `requestedId` that names a live terminal (the spawn is refused with the original still running, so recording it would claim that shell), and `agent.launch` or `workflow.startWorkOnIssue` naming an assistant-only agent from a non-renderer-owned session, which would mint the assistant's pinned bearer. A recipe child of an assistant-only type is refused at launch for every caller, which closes the same route through `recipe.run`, `worktree.createWithRecipe` and a workflow's `recipeId`. That is not a sandbox — a session holding `terminal.new` can still open a shell and run commands in it — but it stops the authority reaching panels the session did not open. The assistant keeps the unscoped tools because sending prompts to agents the user launched is most of its work, and it runs pinned to the window the user is watching.
...
The in-app assistant can carry a session across a close and back. `agentSessionHistory.list` and `agentSessionHistory.resume` are in `full`; resume spawns a pane, so it sits with the other spawn tools. The bookmark tools (`session.bookmarks.list`, `session.bookmark.*`, `session.bookmarkAndClose`) and the recipe-editor handoffs (`recipe.editor.open`, `recipe.editor.openFromLayout`) left MCP with the core/full split: they are the user's to drive from the UI. The line this section used to draw still holds, more strictly than before: **the assistant proposes, the person commits** — no MCP tool writes a recipe, and none opens one for editing.
...
- **Discovery only.** It authorizes nothing. Renderer dispatch still goes through `dispatchActionForWorkspace`, so a viewless `tools/call` for a renderer action stays fail-closed. Confirm-gated tools are withheld either way — `sessionSurface.workspaceBound` is set from _having_ a binding, not from view liveness. What a viewless session **can** call is the main-process surface below.
...
### What a viewless bound session can still call (#12316)

View residency is an LRU capped at 5 (`computeDefaultCachedViews`, RAM-tiered — see `docs/architecture/process-and-window-model.md`), and the cap counts the active view. An orchestrator across dozens of projects therefore holds a binding per workspace but a live view for almost none of them, and originally every one of those calls was refused. Most of them never needed a view.

**`VIEWLESS_MAIN_PROCESS_TOOLS`** (`sessionServer.ts`) names the tools whose execution never touches a renderer: `terminal.waitUntilIdle`, `terminal.waitUntilIdleBatch`, `skills.search`, `skills.load`, `mcp.surface`, `project.runCheck`. These skip the bound confirm ceiling entirely. The ceiling is a question about renderer dispatch, and it answered that question by resolving the bound view's manifest — so it made view residency a precondition for calls that were never going to reach a view. The set sits beside the id constants the short-circuit branches match on, and `sessionServer.test.ts` derives its cases from the exported set — a tool added with no fixture fails coverage, and one that reaches a renderer fails on its own manifest lookup. What that guard proves is the _routing_: each member settles without a view and never reaches `dispatchAction`. It cannot prove a given handler's implementation stays renderer-independent, which is a property of the handler. Tier admission is unchanged and still runs first, so this widens reachability, never authority.

**`daintree://agent/<id>/state`** reads and subscriptions carry no residency probe. The resource answers from the process-global `AgentAvailabilityStore` with no renderer involved. The probe #12082 added closed only half of a gap it could not close: _which_ workspace an agent belongs to is not checked either way, because that needs an agent→workspace map this layer does not have, and a bound session with a live view could always read across it. Closing it properly is #11789's own scoping work and has to apply to both cases.

Worth stating plainly, because it is a real widening and not only a restoration: a viewless bound session can now **establish** an `agentState` subscription, where before it could only keep one it had established while resident. The notification carries the URI, not the state, so it leaks transition timing rather than content — and agent ids name the agent _type_ (`claude`), so the targets were already guessable. Subscription cleanup is unchanged and still holds: revocation, idle expiry, HTTP teardown and drain all remove subscriptions, and the notification callback re-checks session existence. The trade-off is accepted because the alternative is the reachability ceiling this issue was filed about, on a resource that has no renderer in its path at all.

**`terminal.getStatus` has a reduced main-process fallback.** The renderer answer carries the panel-shaped fields — panel identity, `location`, `armed` from `useFleetArmingStore`, and the `lastCheckResult` parsed out of agent stdout — and is used whenever a view exists. It is not strictly richer: `hasPty` runs the other way (below), so a client reads `unavailableFields` per response rather than ranking the two surfaces. When the workspace has no view, `terminalStatus.ts` reads the pty-host records directly (the same source `terminalInventoryPrefetch.ts` builds an inventory from) and answers with agent state, `waitingReason`, `lastTransitionAt`, `lastOutputChangeAt`, `spawnedAt`, `hasPty`, and the optional `recentOutput` tail. Three constraints:

- **The result envelope says which surface answered.** `source` is `"renderer"` or `"pty"`, and `unavailableFields` lists what that surface could not observe — `["armed", "lastCheckResult", "exitCode"]` for `pty`, `["hasPty", "lastOutputChangeAt"]` for `renderer`, which reads `lastOutputChangeAt` only on a call that set `includeOutput` (#12495) — one batched hop beside the tail fetch — and then lists just `hasPty`. `lastOutputChangeAt` (#12428) is the last time the visible screen changed with recognised spinner and timer redraws discounted, tracked per terminal in the pty-host by `OutputProgressTracker`; the wait tools attach it to tracked rows too, read after the wait settles and scoped to a bound session's own workspace. Absence alone would read as "not armed", which main has no evidence for.
- **`exitCode` is the field that looks reachable and is not.** Main does cache exit metadata per terminal in `AgentAvailabilityStore` (#12494), but everything else in this answer comes from the pty-host record, and the store is a second copy fed separately by events — the two can disagree mid-respawn, and splicing one field in from it is a contract change of its own. So the `pty` path declares it unavailable, and `agentState` still separates `completed` from `exited`. (A separate, pre-existing inconsistency on the renderer path: it emits `exitCode: null` for a still-running terminal, which the schema reads as "signal-terminated with no numeric code". Left alone here — it predates this change and belongs to its own fix.)
- **`hasPty` runs the other way (#12336).** `!wasKilled && !isExited`, computed in the pty-host by `mapTerminalInfo`, so the reduced answer is the one that can observe it. A pane that exits cleanly is deliberately preserved, so the record outlives its process and `false` separates "this pane's process ended" from "this pane is gone" — the latter is an id that resolves to no record, and reports `error`. `false` is not proof the process has finished tearing down, either: `TerminalProcess.kill()` sets `wasKilled` before process-tree termination completes, so a poll during teardown reads `false` while descendants may still hold resources. It is a lifecycle fact and not the liveness probe #12336 asked for: an agent that exited to its `keep-open` shell reads `true`, and so does a wedged one. The renderer declares it unavailable rather than forwarding `PtyPanelData.hasPty`, which the type carries but nothing in the renderer ever writes — not `addPanel`, not `statePatcher` on restore or reconnect, not the `onExit` listener — which is why `src/store/fleetEligibility.ts` reaches for `runtimeStatus` instead. Deriving one from `runtimeStatus` would publish an interpretation as a process fact. Populating the panel copy is its own lifecycle work.
- **Ids are checked against the workspace's own inventory before any terminal RPC.** Filtering the records afterwards would answer the same way, but the pty fabric shards by owning project, so a `get-terminal` for a foreign id lands on its owner's shard while an unknown id lands on the default — identical rows, distinguishable latency. Scoping first means a foreign id is never routed.
- **Explicit `terminalIds` are required.** The fleet path filters on `location`, a panel concept with no pty-host meaning; answering it from the inventory would silently redefine what "the fleet" means. A filter-only call keeps its `SESSION_BINDING_GONE`, with the id requirement named in the message so a poller learns the shape that works.
- **Fallback, not replacement, and only for `not-found`.** An `ambiguous` binding is a conflict the user must resolve and a destroyed pin is not a route that comes back, so neither takes the fallback, and neither is offered the explicit-ids advice that could not help it. Records outside the bound workspace, non-PTY panels and the assistant terminal report as `Terminal not found or status unavailable` — one message for three cases, since a failed backend read is real and the wording must not promise the terminal is gone — a bound session must not be able to confirm an id it has no route to.

Only one `*Owned` tool is in any of this. Their ownership checks are all main-side, but six then delegate to a renderer action — `terminal.close` for trash and recovery, `worktree.delete` for its D2 preview, `pilot.openRun` for the switch — and that delegation is renderer work. `terminal.readLastMessageOwned` (#12479) is the exception: it reads a file the agent wrote and dispatches nothing, so it sits in `VIEWLESS_MAIN_PROCESS_TOOLS` and answers for a workspace with no live view.
...
Six tools **act** on the ledger, and none of them is the general primitive:
...
- **`terminal.revealOwned`** (`danger: "safe"`, #12315) — checks ownership, then dispatches `pilot.openRun` to bring the user to the panel. It differs from the other three in every way that matters. It was the first entry to **keep** its record (`releasesOwnership: false`): navigating to a panel is not a claim that it stopped existing, and dropping the record would cost the session the authority to reveal it again or clean it up at all. It is the only one whose delegate spells the id differently (`delegateIdArg` rebuilds `terminalId` as `pilot.openRun`'s `runId`), and the only one that supplies a `workspaceId` the caller never sent — read from the ledger record main wrote when the panel was created, falling back to the session's bound workspace and omitted entirely when neither is known, because `pilot.openRun`'s own default (the executing view's workspace) is the only honest guess left. And it is the one place on the external surface that deliberately moves the user: the switch runs in the active view of the window that already _holds_ the destination workspace — not the session's bound view, and not the destination's own cached view, which cannot snapshot the drafts of the view it would replace — and then raises that window, resolved from the view the dispatch ran in rather than looked up again afterwards. The response reports whether the raise actually landed instead of assuming it, so a failed raise reads as a failed reveal.
- **`terminal.interruptOwned`** (`danger: "safe"`, #12338) — checks ownership, then dispatches `terminal.interrupt` to write a double-Escape to the panel. It was the step this surface did not have between waiting for an agent and destroying it: submitted text queues behind the very turn it was meant to stop, because an agent mid-turn is not reading its prompt. It is the second entry that **keeps** its record (`releasesOwnership: false`) — a stop is not an end, so the session keeps the authority to stop the panel again and to close it afterwards. A signal was never the alternative: node-pty's `kill()` reaches our wrapper shell, which carries `trap : INT` and swallows it before the agent sees anything, so the mechanism is a keystroke. What it refuses matters as much as what it sends. An agent whose own CLI advertises a different cancel key (Goose, `(Ctrl+C to interrupt)`) is named and refused rather than written to, and one that is not observed mid-turn is refused rather than sent a stray Escape that Claude's TUI would read as opening the rewind menu. And it never reports an interruption: `batchDoubleEscape` is a one-way `ipcRenderer.send` with no reply, so every outcome it can report is one it asked for — `status: "requested"`, or `"requested-unverified"` when the target's CLI names no interrupt key at all — and it stops there. `terminal.waitUntilIdle` is not the confirmation either — it answers `idleReason: "unknown"` for any terminal it has no agent id for.

- **`terminal.sendCommandOwned`** and **`terminal.injectOwned`** (`danger: "safe"`, #12407) — check ownership, then dispatch `terminal.sendCommand` and `terminal.inject`, so the atomic submission, its `submissionToken` receipt and the injection path are the shipped ones. Both keep their record: submitting to a panel is not a claim it stopped existing. The submission is the only entry with `forwardArgs` — it copies `command` by name, then writes the checked id last, so no forwarded field can name a different target; the injection forwards nothing, because the context it writes is the active worktree's and never the caller's. They replaced the unscoped pair on `MCP_EXTERNAL_TIER_TOOLS`, and they are the only way a non-renderer-owned session types into a terminal that is already open — launch prompts, a new terminal's command and the owned interrupt are separate, narrower routes.

- **`terminal.readLastMessageOwned`** (`kind: "query"`, `danger: "safe"`, #12479) — checks ownership, then runs in main instead of delegating: `OWNED_RESOURCE_TOOLS` entries are an explicit union of a `renderer` executor (`delegateTo` and its argument rebuild) and a `main` executor naming an `OwnedMainExecutors` dep, and the main arm runs straight after the gate, before any renderer manifest is resolved, under the shared audit `finally`. It returns what the agent last wrote to its own transcript: the text blocks of the last main-chain assistant `message.id` that has any text, and the `tool_use` blocks with no matching `tool_result` later in the file — which is how a question with options, never text, comes back. The caller sends a terminal id plus optional `maxBytes`, `messageIndex` and `cursor` (#12496), which the main arm's `readOptions` (`lastMessageArgs.ts`) validates by hand because no schema parse runs on a main-executed call; the executor receives those and nothing else. The session id and cwd come from the pty-host record, and the store is the one remembered at the pane's own spawn (`getClaudePaneProjectsRoot`), never Daintree's own `~/.claude` — an uncertain store is `store-unknown`. `ClaudeSessionReader` reads backwards from the end in 256 KiB steps to an 8 MiB ceiling through one verified descriptor (`lstat`, then `open` with `O_NOFOLLOW`, then `fstat` against the checked inode), decodes only whole lines, and reports `search-cap-reached` rather than an older reply when the ceiling comes first. The text is capped at 24 KiB of escaped bytes keeping the end, or at a requested `maxBytes` of 1–48 KiB. A longer message is paged backwards with `message.nextCursor`, a stateless cursor naming the message by id and a digest of the text before the page, so a message that changed answers `message-not-found` rather than splicing in another's text, and one that has left the 8 MiB read answers `search-cap-reached`; `messageIndex` reaches up to 20 replies back within the same 8 MiB read. `input` rides only on `AskUserQuestion` and is omitted whole past 8 KiB, and the whole result is held under the response cap so the structured half is never dropped. Claude only for now: every other agent is `provider-mismatch`, and the result names its `provider`. Everything in it is an observation of the file — an unanswered tool use does not prove the CLI is waiting, and a permission prompt is never in the file — and the audit record keeps its shape only (see [Guard rails and observability](#guard-rails-and-observability)). It keeps its record: reading a panel is not a claim it stopped existing.

The generic `terminal.close` and `worktree.delete` stay off `MCP_EXTERNAL_TIER_TOOLS`, and so does `pilot.openRun` — it is reachable only through the `terminal.revealOwned` short-circuit. The same goes for `terminal.interrupt` and `fleet.interrupt`: the unscoped interrupts reach any pane a listing names, including the user's own mid-turn. All seven `*Owned` tools execute as main-process short-circuits because the ledger is keyed by MCP session or bearer principal, neither of which the renderer ever sees.

One tool **reads** it: `terminal.list` takes an optional `owned: true` (#12308), which returns the normal listing intersected with this session's terminal records. It answers "which of these did I create?", not "which of these can I close" — the two predicates differ in both directions and neither contains the other. The listing still applies `worktreeId`, `location` and the default trash/background exclusions, which cleanup does not, and a delegated `terminal.close` reports nothing closed for a panel already in the trash — which an explicit `location: "trash"` listing will happily show as owned. The ownership half is shared, though: the gate and the listing call one predicate (`ownedRecordFor` in `sessionServer`), workspace-mismatch check included, so a row the listing marks owned is one the `*Owned` tools accept (#12487). It is a filter argument rather than a tool, because a boolean does not earn a second entry in the tools manifest every model turn pays for. The mechanism is two coupled halves, and the coupling is the point: `prepareTerminalListDispatch` takes the flag off the arguments forwarded to the renderer and returns the filter decision from the same destructure, and the result is intersected with the ledger at the shared `outcome` assignment, beside `filterIntrospectionResultForSession`. The renderer's own `run()` throws on any defined `owned`, `false` included — it cannot resolve ownership, so returning the unfiltered list would answer "which of these did I create?" with "all of them", which is exactly the quiet no-op the argument exists to avoid. Only an actual boolean is consumed; a non-boolean is forwarded untouched so the renderer's schema rejects it rather than having the strip launder it into a legal request. The filter keeps rows, never manufactures them: a record with no matching row in the listing yields nothing, because the listing is what says which panels exist and the ledger only says who created them. Where it parts company with `filterIntrospectionResultForSession` is a payload it cannot read — an empty list is a substantive answer here ("you created none of these"), so an unreadable listing fails the call instead of being emptied.

A cleanup-tool ownership refusal is `RESOURCE_NOT_OWNED`, and it is deliberately one code for three situations — the id never existed, it belongs to another session, or it belongs to the user, a plugin, or the in-app assistant. Distinguishing them would make the cleanup tools an enumeration oracle. The check runs before the delegated dispatch, so a refused call never reaches a renderer.

**Ownership lasts as long as the credential that earned it, and disconnect does not clean anything up.** An api-key session holds its own records: `clearSessionBinding` drops them alongside route, context, origin and workspace, and `drain` drops every session's — that revokes _authority_, not the resources. Terminals and worktrees a disconnected client created stay exactly where they are: the session ending is not a decision to destroy work the user can still see, and a reconnecting api-key client gets a new session id with an empty ledger. Cleaning up after a client that went away is the user's call, through the normal UI. Ownership is recorded for every tier rather than only `external`, because "this session created it" is a fact about the session rather than about its privileges.

A per-pane bearer is different (#12487). Daintree mints it, writes it 0600 under `userData`, and revokes it when the PTY exits, so it identifies the pane rather than a connection. `McpPaneConfigService` gives each such token its own ownership principal, and both handshakes bind the new session to it (`ResourceOwnershipLedger.bindPrincipal`), resolved from the bearer server-side and never from anything the client sends. The session's records are held by the principal: teardown and `drain` only unbind the session, so a pane that reconnects after a transport error, an idle reap or a server restart finds everything it launched, and two live sessions on one bearer share one set of records. `revokePaneConfig` tells the MCP server in the same synchronous step that deletes the token — PTY exit, spawn failure, re-prepare and `revokeAll` alike — and `revokePrincipal` drops the records and marks the principal dead, so a session still bound to it owns and records nothing until it ends. A relaunch mints a fresh token and a fresh principal, so it never inherits its predecessor's records. Assistant-pane bearers are pane tokens too and follow the same rule; help sessions and api keys do not.

`terminal.list { owned: true }` reports this honestly: after an api-key client reconnects it comes back empty, because the new session genuinely owns nothing. That is what lets such a client say so. The api key is deliberately not the durable identity: one external API key is shared by every client (`mcpClientConfigs.ts`), so keying ownership off it would collapse every external client into a single owner — a wider boundary, not a narrower one.
...
Smaller consequences worth knowing. The owner is captured when a call is admitted. For an api-key session, a creation that lands _after_ the session was torn down records nothing — otherwise a dead session's authority would come back and, worse, claim the id away from whoever records it next. For a pane bearer it still records, because the principal outlives the transport, unless the bearer was revoked meanwhile. A release drops only the record the gate checked, so a cleanup that finishes after another session on the same bearer recorded the id again leaves the newer record alone. And `worktree.deleteOwned` is deliberately absent from the dedup allowlist that `worktree.delete` sits in: worktree ids are paths, create → delete → recreate is a supported workflow, and a cached delete would report success for a second, genuinely different worktree.
...
`minimumPermittingTier(toolId)` computes the lowest non-`external` tier that would permit a denied tool, so the renderer's "Approve once" / "Always allow" buttons target the _narrowest_ sufficient tier. With two in-app tiers that is always `full` for a `core` session; the origin is part of the question, since an owned twin is placed only on the non-renderer-owned surface.
...
**`getTier` returns `McpTier | null`, and `null` means "no live transport" (#11799).** `core` is not a floor — `TIER_ALLOWLISTS` gives `external` its own allowlist rather than a subset of `core`, so defaulting a vanished session to the lowest in-app tier would _widen_ a revoked external bearer onto tools its allowlist withholds (46 of them against the old `workbench` default). Every authorization path (`tools/list`, `tools/call`, and all four resource handlers) resolves the tier once at entry and fails closed on `null` with `SESSION_GONE`. Discovery throws rather than answering with an empty list, which would read as a legitimate empty surface.
...
A tool with no meaningful active-worktree default opts out with `{ requireSelector: true }`, which rejects a call that supplies no selector rather than retargeting it. Use it wherever the fallback would be silently wrong: a destructive write (`artifact.applyPatch` — a "if none given, use the active one" default on a destructive submit is the no-silent-fallback-defaults rule from [destructive-action-safeguards.md](./destructive-action-safeguards.md), the #7880 root cause), or a tool that anchors on the REPO ROOT while the active worktree is usually a linked one (`worktree.create`, `worktree.getDefaultPath`).
...
## Worktree creation and readiness

`worktree.createWithRecipe` is the **managed worktree creator**, not a recipe launcher — the recipe is optional and always was, and everything else the tool does happens regardless. Its id is unchanged (renaming one is a breaking change for every external caller, and the id is wired through both allowlists, the dedup set, the resource-ownership extractors and the generated external manifest); the title and contract carry the correction.

**Creation mode is explicit.** Arguments are a `source` discriminated union with three arms:

| `source.kind` | Fields | Behaviour |
| --- | --- | --- |
| `newBranch` | `branchName`, `baseBranch?`, `fromRemote?`, `collisionPolicy?` | Creates a branch off the base. |
| `existingBranch` | `branchName` | Checks out that exact local branch. Never suffixed. |
| `pullRequest` | `pullRequestNumber` | Fetches and checks out the PR's head branch. |

The union is nested under `source` rather than being the root schema, and that is load-bearing: `buildToolInputSchema` forwards a generated schema only when its root is `type: "object"`, and Zod emits a root `oneOf` with no `type` for a top-level union — which would be replaced by an EMPTY object schema, shipping a tool that advertises no parameters at all while still requiring them. Nested, the root stays an object and `toWireSchema` preserves the combinator. `actionDefinitions.quality.test.ts` asserts object-rooted input for every tier-exposed tool so that substitution can never fire unnoticed.

The flat predecessor took five independently optional top-level fields whose legal combinations lived only in `run()`. Zod refinements do not survive `z.toJSONSchema`, so the manifest advertised `{}` as a valid call and only a dispatch could teach otherwise. It also mixed modes that are not compatible: `baseBranch` and `fromRemote` mean nothing when an existing branch is being reused, and nothing said so.

**Collision handling belongs to the host.** `collisionPolicy` (`suffix`, the long-standing default, or `error`) reaches `WorkspaceService.createWorktree`, where collision detection rides the atomic `git worktree add` failure. A renderer-side available-name lookup cannot implement either policy because it reserves nothing — the name can be claimed between the check and the create, and the host's own recovery may suffix further or reuse a stale branch. The renderer still calls `worktree.getAvailableBranch` under `suffix`, but only as a NAMING HINT for the worktree directory; the result reports `requestedBranch` and `effectiveBranch`, and `effectiveBranch` travels back **with the create response** rather than being read out of the worktree store. That distinction is load-bearing: worktree rows reach the renderer over the project's own port while a create answers on the parent-process transport, so there is no ordering between them and a store read immediately after a create can miss the row entirely. `worktree.create` returns `{ worktreeId, branch }` for the same reason. Under `error` the lookup is skipped entirely so the host sees the exact name that collided.

`existingBranch` bypasses the lookup unconditionally, which is the bug the mode was added for: the flat shape resolved an available name _before_ it read `useExistingBranch`, so asking to reuse `topic` produced `newBranch: "topic-2"` — a branch that by construction does not exist, since the lookup's whole job was to find a free name.

**Creation reports as soon as git is done, not when the worktree is usable.** Config copy, submodule init and the project's setup script run in a fire-and-forget tail afterwards. `Worktree.setupStatus` (`pending` → `running` → `ready` | `failed` | `timed-out`, with the failing `stage`) records that tail and is written on every path _including_ the no-setup-commands one. It is deliberately separate from `lifecycleStatus`, which is a single generic slot for whichever lifecycle phase ran last — a later resource-pause or teardown overwrites a setup result in it, and it is never written at all when a project declares no setup commands, so its absence means three different things.

The status is held in memory by the monitor, so a worktree this host process did not create has none. That reads as **`unknown` on the wire, never `ready`** — saying otherwise would be an interpretation of silence.

Every worktree listing row carries the state alone (`setupState`) rather than the whole status: the object rides every row into model context, and the stage and failure text only matter once a caller has picked a worktree. `worktree.waitUntilReady` is the detail view and the waiter; a zero timeout is the non-blocking read, so there is no separate getter. It is capped at 25s because the renderer dispatch path times out at 30s — a longer wait would be killed mid-flight and reported as a transport failure rather than as "still running", which is the one answer it exists to give honestly. Setup scripts routinely outlast that, so the contract is "call again", and running out of time is reported as `timedOut` beside a live state rather than as an error.

`worktree.waitUntilReady` is in `core` and deliberately NOT on the external allowlist, which is at its tool-count ceiling. External callers get readiness through `setupState` on `worktree.list`, which is already on that surface.

**Recipe terminals do not wait for setup.** They start as soon as git is done, which is the long-standing behaviour and is why the creation result reports `setupState`: a caller that needs a fully initialized tree polls `worktree.waitUntilReady` before dispatching work into the terminals.
...
A tool registry is charged to the model's context on every request, so an oversized one is not merely wasteful — it gets silently mangled. The `external` surface stood at 100 entries and ~128 KB of schema before #11585, past what MCP clients tolerate: Cursor caps the tool count across all connected servers and truncates the overflow without telling anyone, while GitHub Copilot's 128-tool cap is a hard error. Truncated or rejected, the client was choosing our tools for us. It now sits at thirty-one. Two regression thresholds bound it on **both** axes, because either one alone is gameable:

- **Tool count** — `EXTERNAL_BUDGET_MAX` and a floor of 15, asserted in [`tierAuth.test.ts`](../../electron/services/mcp-server/__tests__/tierAuth.test.ts). The floor is as load-bearing as the ceiling: a surface that quietly erodes to nothing is also a regression. The roster itself is [`shared/config/mcpExternalTierAllowlist.ts`](../../shared/config/mcpExternalTierAllowlist.ts); read the ceiling from the test rather than from a number here.
- **Aggregate wire bytes** — `MAX_EXTERNAL_PAYLOAD_BYTES` for the external roster and `MAX_COHORT_PAYLOAD_BYTES` for the full in-app surface, asserted in [`mcpWireBudget.test.ts`](../../src/services/actions/__tests__/mcpWireBudget.test.ts). Each sums description + input-schema + output-schema bytes per tool, so counting tools alone can't be gamed by a couple of dozen tools carrying novel-length descriptions or sprawling schemas. Per-tool ceilings sit underneath: 400 B on a tool description, 1,500 B on its `parameters`, and a 120 B _floor_ on the description (`actionDefinitions.quality.test.ts`) so a tool can't be shrunk into uselessness.

Both are **test thresholds, not runtime constants** — nothing rejects a call or a listing for exceeding them; the unit suite fails in CI instead. Both derive their input from the live registry rather than a fixture, so neither can drift from the gate it budgets. Note what they do _not_ measure: tool names, annotations, `_meta.examples` and JSON framing all ride a real `tools/list` response without moving either number, so the thresholds are a regression tripwire on the roster, not a bound on the serialized listing. Every ratchet raise carries a written reason in the test file — the rules for what counts as one are in [mcp-context-condensation.md](./mcp-context-condensation.md#raising-a-ratchet).

The in-app tool sets (`core` / `full`) are outside **these tests**, but they are not exempt from client limits, and the core/full split exists because of them: the old `action` default carried 142 tools and the `system` tier 181, past the Copilot figure above, all re-sent every turn. `core` is now a few dozen tools and `full` a few dozen more. Derive the current totals from `HELP_TIER_CUMULATIVE` in [`shared/config/helpAssistantTierAllowlists.ts`](../../shared/config/helpAssistantTierAllowlists.ts) rather than trusting a number written down here, for the same reason the external count isn't restated above.
...
Read that as a per-block cap, not an envelope cap. A promised `structuredContent` is measured against the same 50 KiB separately, so a response carrying both halves can exceed 50 KiB in total. One path bypasses the helper entirely: protocol-level JSON-RPC errors, which never reach it. (`browser.captureScreenshot` used to return an uncapped base64 image block here; it left MCP with the core/full split.)
...
The `forge.*` tools are how an agent reads and mutates the project's issues, pull requests, and reviews over MCP. Every action is defined in [`src/services/actions/definitions/forgeActions.ts`](../../src/services/actions/definitions/forgeActions.ts); the `run()` bodies stay provider-agnostic and provider routing (GitHub, GitLab, …) is resolved at the IPC layer — reads in `electron/ipc/handlers/forgeData.ts`, settings and auth in `forgeSettings.ts`, with `forgeResolution.ts` resolving a cwd to a provider for both. For how a provider is selected and normalized, see [`forge-provider-abstraction.md`](./forge-provider-abstraction.md) — this section documents only the MCP-facing contract (id, args, tier, `danger`, rate limit, dedup). Every forge action but one takes the eager `tools/list` path and surfaces whenever the caller's tier permits it; the exception is `forge.validateToken`, which is `mcpVisibility: "hidden"` and on no tier at all (see below).

The tables below are the authoritative `forge.*` surface, derived from three sources that must stay in sync: `forgeActions.ts` (`kind`, `danger`, `argsSchema`), `shared/config/helpAssistantTierAllowlists.ts` (tier membership — five reads in `FULL_TIER_ADDONS`, everything else on no tier), and `electron/services/mcp-server/shared.ts` (`RATE_LIMIT_TOOL_MAP` for the bucket, `MCP_DEDUP_ALLOWLIST` for dedup). The external surface is no longer one of them: `shared/config/mcpExternalTierAllowlist.ts` contains no `forge.*` ids at all (#11585), so forge is a help-assistant-only surface and tier membership is decided entirely by the two lists above. Location arguments follow the repo-wide convention below: every repo-scoped forge action takes `worktreeId?` / `worktreePath?` (with `cwd?` still accepted as a legacy alias), the browser-open list actions (`forge.openIssues`/`forge.openPRs`/`forge.openCommits`) and `forge.openRepo` take `projectId?` / `projectPath?` and resolve against the current project's path, and `forge.validateToken` takes no location arg — it is also on no tier at all, see below.
...
### Forge reads (`full` tier)

`forge.listIssues`, `forge.listPRs`, `forge.getIssue`, `forge.getPR` and `forge.getCIStatus` are in `FULL_TIER_ADDONS`, so they are reachable at `full` — but not at `core` or `external`. The other four rows below are on no tier since the core/full split; they stay documented because the actions still serve the UI. All are `kind:"query"`, `danger:"safe"`, on the `standard` bucket, and not deduped.
...
| `forge.getPRs` | `prNumbers` (2-20), `cwd?` |
...
- **`view` defaults to `summary`**, which drops each row's `body` and `rawData` (the verbatim provider node) and flattens actors and labels to their names, keeping what is needed to choose an item — including `linkedPR`, which answers whether a PR is already working the issue. Pass `view: "full"` for the complete provider object. This is a runtime projection built in `run()`, not a schema effect: `resultSchema` here describes the widest row the action can build, so within that shape a field stops being sent only when `run()` stops building it. (`dispatch()` _does_ parse results against `resultSchema` since #11539 — that is what strips `rawData` from the singular PR reads — but a schema that permits both `view` shapes cannot narrow one into the other.) Neither list action sets `mcpOutputSchema`, so their `resultSchema` is declarative metadata for readers of the definition rather than an advertised MCP `outputSchema` — deliberately, since `sessionServer` emits `structuredContent` _alongside_ the serialized text body, which would send every row twice and undo the projection's whole point.
...
`forge.getCIStatus`, `forge.getChecks`, `forge.getPR` and `forge.getPRs` set `mcpOutputSchema: true`, so they are the forge reads advertising an MCP `outputSchema` and returning `structuredContent`. All four wrap their result — as `{ ciStatus }`, `{ checks }`, `{ pr }` and `{ results }` — rather than returning it bare: `buildToolOutputSchema` forwards only object-typed schemas, so a top-level nullable would silently advertise nothing. The handler projects the provider's `CIStatus` down to the roll-up fields and drops `rawData`/`freshnessToken`/`notModified` — `rawData` in particular is populated on a network fetch but `null` on a cache hit, so forwarding it would make the response depend on cache state. `forge.getChecks` drops per-check `rawData` too, though for a different reason: it bypasses that cache entirely, so its `rawData` is always populated — it is dropped because the verbatim provider node is not part of the advertised cross-provider schema.

**`forge.getPR` and `forge.getPRs` are singular and plural forms of the same lookup, not alternatives.** One known number takes the singular; two to twenty take the plural, which is one GraphQL request on the GitHub provider rather than N. That is a property of the provider, not a guarantee of the tool: a provider without `batchLookups` falls back to a singular read per number, which is also why the ceiling is 20 — twenty sequential round trips already sit uncomfortably inside the 30s dispatch budget. Neither is a discovery tool — numbers you do not already have come from `forge.listPRs`. Both wrap their payload (`{ pr }` and `{ results }`) for the object-root reason above; a bare `PR | null` emitted `anyOf` and advertised nothing at all.

The plural form reports **per-number status, and the three values are not interchangeable**: `found` carries a `pr`; `not_found` means the forge was asked and has no such PR; `unresolved` means nothing was learned — a failed request or a rate limit — and carries a `reason`. Acting on `unresolved` as though it were `not_found` closes work that exists. The IPC handler preserves the distinction the provider contract already draws: `batchLookups.findPRsByNumbers` returns an explicit `null` for "no such PR" and simply omits a key whose chunk never resolved. A provider with no batch capability falls back to per-number `getPR` rather than reporting an empty answer it has no basis for.
...
### Forge writes and commands (no tier)

Every forge write and browser opener — `forge.open*`, issue and PR assignment, reviews, create/close/reopen/merge/edit, comments and labels — is on no tier since the core/full split. They used to sit at `system`, and nothing ever called them over MCP: an agent driving Daintree has `gh` or its provider's own tooling, and the assistant hands an approved issue draft to the user rather than filing it. They remain ordinary actions for the UI.

`forge.validateToken` is on no tier for a stronger reason — its argument _is_ a raw forge access token, and a tool argument is model context by construction: whatever redaction the audit log and dispatch summaries apply afterwards, admitting the tool means the credential has to be composed in the model channel before any of that runs. It also carries `mcpVisibility: "hidden"`, so an allowlist edit cannot re-advertise it by accident. Token entry stays a UI-owned flow — the provider settings tab dispatches it as `source: "user"`, which no tier gates. `tierAuth.test.ts` holds both gates.

No forge action — read or write — is reachable at the `external` tier (#11585). Adding a forge read to MCP is an edit to `FULL_TIER_ADDONS` and nothing else; the `forge.rateLimit` test and the tier snapshots guard against drift.

## Submitting input (`terminal.sendCommand`)

`terminal.sendCommand` is how an agent puts text into a terminal — a shell command for a plain pane, or the next prompt/turn for an agent pane (Claude/Codex/Gemini/…). The name is shell-flavoured for historical reasons; the operation is "submit this text to whatever owns the PTY". It routes through `terminalClient.submit` → `TerminalProcess.performSubmit`, which is the load-bearing detail: the body is delivered as **one atomic bracketed paste** (or, on agents without bracketed-paste support, the body with interior `\n` rewritten to the agent's soft-newline) followed by **exactly one Enter**. So multi-line text is safe — embedded newlines insert line breaks and never prematurely submit a partial message — and a trailing newline is the submit Enter (N trailing newlines send N times). The body is also **neutralised before any protocol bytes are added**: `normalizeSubmitText` folds line endings and then replaces every remaining C0 control and DEL with its Control Pictures glyph, on all three branches, so text this call carries reaches the agent as text and not as terminal input. The delimiters, the soft newline and the Enter are added afterwards by the code that means them; the raw keyboard path (`write`) is deliberately untouched, because a keystroke is supposed to be a control character. The call is **fire-and-return**: it does not block on the agent's reply, so it carries none of the `waitUntilIdle` conversation-hostage constraint below. The right decomposition for "send and read the answer" is `sendCommand` then a separate `terminal.getStatus`/`waitUntilIdle` poll, not a single blocking call. `sendCommand` sets `denyPluginDispatch` — plugins must declare `agent:input` and use `host.sendToActiveAgent(...)` rather than routing input through this `safe` action (#10558). Over MCP it reaches any panel, so only the assistant's own sessions are offered it; any other session whose tier admits terminal input submits through `terminal.sendCommandOwned`, which delegates here for a terminal that session created (#12407).
...
## Terminal notices (`terminalNotify.ts`)

A wait holds the caller's conversation open and is capped at 60s for in-app sessions, and MCP has no way to start a turn in an idle client, so an orchestrator waiting on a long job has to spend turns polling. Notices close that gap. The orchestrator asks per call — `notify: true` on `agent.launch`, `terminal.sendCommand` or `terminal.sendCommandOwned`, or `terminal.notifyWhenIdle({ terminalId, note? })` for an agent that is already working — and ends its turn. When the target next leaves `working`, Daintree types a notice into the orchestrator's own prompt. Its first line is fixed wording: the terminal id, what was observed (`now waiting at its prompt`, `now waiting on an approval`, `exited (code 0)`, `was closed`), whether a handback was seen, and the caller's note. Below it, unless the caller passed `replyLines: 0`, comes the target's last screen lines (40 by default, up to 200), in a fence longer than any backtick run inside and labelled as terminal output, not instructions. With a handback the quote ends at the agent's marker, which drops the composer rows a TUI draws below its reply; an `END-` row whose own row or the row above carries the `<summary>` placeholder is the echoed instruction, not the marker. Replies are capped at 6,000 characters each and 20,000 per notice, newest kept, and echoed terminal-query replies (`^[_G…`, `^[[?…c`) are stripped. The quote is read when the notice fires and read again at delivery unless the target is working again, because some CLIs pause before printing an answer they have already drafted. Reading it here is what saves an orchestrator a `terminal.getStatus` per agent per round, and Claude Code receives the multi-line notice as a paste, so its host frames it as pasted content.

**Routing.** The line goes to the caller's own terminal, resolved from its credential and never from its arguments: a pane bearer's terminal, keyed by its ownership principal so a reconnect keeps its notices, or the terminal an assistant lane's help session is bound to. Three lanes in one project are three bearers and three owners, so a notice only ever reaches the lane that asked. An api-key client has no pane: it is refused `NOTIFY_NOT_ELIGIBLE` before anything is sent, and `MCP_EXTERNAL_OMITTED_ARGS` keeps `notify` off its listing.

**Project scope.** A target must be a running agent terminal in the caller's project; a missing id and another project's id get the same refusal. Delivery goes from main straight to the pty-host, so it does not depend on which project is on screen, and the pane chip's state is pushed only to that project's views. A lane with notices pending is not hibernated.

**When it fires.** The first `agent:state-changed` out of `working` that stays out for `NOTIFY_TARGET_SETTLE_MS`, an agent state of `exited` from any state (a CLI quit from its own trust dialog never worked), or the target's exit, kill or trash. A pane closing a target itself (`terminal.close`, `terminal.closeOwned`) drops its own pending notice for it, and a new send or key press to a target drops that target's fired-but-undelivered notice, which would otherwise arrive after the new prompt and read as its answer. `notify` on `terminal.sendKeys(Owned)` counts settles from the key press, so answering a dialog also arms the notice for the turn it unblocks. A send's notice is set up before dispatch so no state change is missed, then counts settles only from its submission's `pty_written` time, so the end of what the target was already doing is never reported as the end of the new prompt; a submission that is never written is reported as such. A notice fires once and is gone.

**Delivery** reuses the guarded submit lane: only an agent waiting at an empty prompt with nothing typed since it settled is typed into (`evaluateWakeGate`), the host re-checks that when the line reaches the lane and drops the Enter if anyone types first, notices that fire together share one line, one line is outstanding at a time, and a line that cannot be confirmed written is never retried blind — it goes out again after the pane's next finished turn. The pane header and the assistant footer show a chip while anything is pending, with Stop.
...
`fleet.getRunStatus` is the read-only MCP window onto that run: the `full` tier and the external allowlist, `readOnlyHint`, structured `outputSchema`, no args. It reports the run owned by the dispatching window and never dispatches anything. The broadcast itself is deliberately NOT MCP-exposed (`terminal.bulkCommand` stays renderer-only on every tier) — an external orchestrator fans out `terminal.sendCommandOwned` per terminal it launched and watches with batched `terminal.getStatus` / bounded `waitUntilIdleBatch`, per the `triage_terminals` prompt. The in-app assistant's own `CLAUDE.tasks.md` guidance uses the unscoped `terminal.sendCommand`, which its renderer-owned sessions keep.
...
- **`auditLog.ts`** (`AuditService`) — ring buffer of `McpLogRecord`s persisted to the `mcpServer` store key, debounced (`AUDIT_FLUSH_DEBOUNCE_MS`). Every gate outcome above is recorded. A successful result's `resultSummary` passes through `projectAuditResult` (`auditResultProjection.ts`) before the generic summarizer: a tool whose result is conversation registers a host-owned projection there, and `terminal.readLastMessageOwned`'s keeps status, provider, character count and tool names — never the text or a question's input, which the secret scrubber would not recognise (#12479). Derives anomaly signals: first-seen `(tool, tier)` combos, latency drift, failure clusters, p95 z-score. Grant lifecycle events and `tier.decayed` are appended here too.
...
### Answering an agent's dialog (`terminal.sendKeys`)

A send types text and then Enter, which picks whatever option a CLI's dialog has highlighted, and the defaults differ: Claude Code's folder-trust dialog highlights "No, exit", Codex's "Trust and continue". `terminal.sendKeys` (assistant, `core`; `terminal.sendKeysOwned` for other callers) writes named keys only, spaced 80 ms apart, and refuses a missing or input-locked terminal. `choose: "<label>"` plans the keys itself: `planChoice` (`shared/utils/terminalChoice.ts`) finds the labelled option and the highlighted row near it on the screen, counts only option rows (a deeper-indented row is a wrapped description), and presses the arrows and Enter, refusing rather than guessing when it cannot see both. The result carries the screen's last lines 700 ms after the keys, so a wrong key shows in the same call.

### Daintree's own assistant sessions

Help and assistant-pane sessions get one-line `instructions` (`MCP_ASSISTANT_SERVER_INSTRUCTIONS`); their own prompts carry the rules, and Codex in code mode repeats a server's instructions inside every tool description, which cost ~1 KB per tool each time the model looked one up. A Codex help session is launched with the session folder trusted for that process only (an inline `projects` table keyed by the canonical path) and `default_tools_approval_mode="approve"` on Daintree's three servers, matching the Claude lane's allow list; neither touches `~/.codex/config.toml`.
