...
**Unscoped terminal input is reserved for renderer-owned origins (#12407).** The ladder tiers are not only the assistant's: a Claude pane launched in a project with Daintree MCP enabled gets its own bearer at the project's tier, with origin `external`. `terminal.sendCommand`, `terminal.inject` and `copyTree.injectToTerminal` accept any panel id a listing returns, so granting them to that session let an agent in a read-only sandbox type into a neighbouring shell and run commands as the user. `RENDERER_OWNED_ORIGIN_ONLY_TOOLS` names them, and `getTierPermittedActionIds(tier, rendererOwnedOrigin)` returns `NON_RENDERER_OWNED_TIER_ALLOWLISTS[tier]` — the same surface with those removed — for every session that is not `help` or `assistant-pane`. Discovery, the call gate, `mcp.surface` and the target policy all read that one selector, and it defaults to the narrower set. Such a session reaches terminal input only through `terminal.sendCommandOwned` and `terminal.injectOwned` (see [Resource ownership](#resource-ownership-11909)). That is not a sandbox — a session holding `terminal.new` can still open a shell and run commands in it — but it stops the authority reaching panels the session did not open. The assistant keeps the unscoped tools because sending prompts to agents the user launched is most of its work, and it runs pinned to the window the user is watching.
