...
| `resourceOwnership.ts` | Which session created which terminal/worktree, written only from trusted post-dispatch results. Backs `terminal.closeOwned` / `worktree.deleteOwned`, and `terminal.list`'s `owned` filter reads it. See [Resource ownership](#resource-ownership-11909). |
...
Two tools **act** on the ledger, and neither is the general primitive:
