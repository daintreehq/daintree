...
**`terminal.getStatus` has a reduced main-process fallback.** The renderer answer is the full one — panel identity, `location`, `armed` from `useFleetArmingStore`, and the `lastCheckResult` parsed out of agent stdout — and is used whenever a view exists. When the workspace has no view, `terminalStatus.ts` reads the pty-host records directly (the same source `terminalInventoryPrefetch.ts` builds an inventory from) and answers with agent state, `waitingReason`, `lastTransitionAt`, `spawnedAt`, `hasPty`, and the optional `recentOutput` tail. Three constraints:
