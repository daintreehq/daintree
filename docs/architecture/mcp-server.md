...
Resolution goes through `getWebContentsForProject`, a registry read with no attach/thaw/focus/switch side effects. **Exactly one** live view must own the workspace; zero and many both refuse. A refused handshake returns HTTP 400 with a JSON-RPC error at `MCP_HANDSHAKE_REJECTED_CODE` (`-32002`, distinct from the `-32001` "Session not found"), a stable `error.data.code` (`WORKSPACE_SELECTOR_MISMATCH` / `_INVALID` / `_NOT_ALLOWED`, `WORKSPACE_NOT_FOUND`, `WORKSPACE_AMBIGUOUS`), and **no** `Mcp-Session-Id` — no session state is created at all.
