...
`terminal.closeOwned` is deliberately **not** in any of this. Its ownership check is main-side, but the close delegates to `terminal.close` for trash and recovery, which is renderer work.
