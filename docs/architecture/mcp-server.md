...
One tool **reads** it: `terminal.list` takes an optional `owned: true` (#12308), which narrows the listing to what the calling session created — the same set `terminal.closeOwned` will act on. It is a filter argument rather than a tool, because a boolean does not earn a second entry in the tools manifest every model turn pays for. The mechanism is two coupled halves, and the coupling is the point: `prepareTerminalListDispatch` takes the flag off the arguments forwarded to the renderer and returns the filter decision from the same destructure, and the result is intersected with the ledger at the shared `outcome` assignment, beside `filterIntrospectionResultForSession`. The renderer's own `run()` throws on any `owned` that reaches it, `false` included — it cannot resolve ownership, so returning the unfiltered list would answer "which of these did I create?" with "all of them", which is exactly the quiet no-op the argument exists to avoid. Only an actual boolean is consumed; anything else is forwarded untouched so the renderer's schema rejects it rather than having the strip launder it into a legal request. The filter keeps rows, never manufactures them: a record whose panel the user has since closed simply does not appear, because the listing is what says a panel exists and the ledger only says who created it.
