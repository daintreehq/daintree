...
**Unscoped terminal input is reserved for renderer-owned origins (#12407).** The ladder tiers are not only the assistant's: a Claude pane launched in a project with Daintree MCP enabled gets its own bearer at the project's tier, with origin `external`. `terminal.sendCommand`, `terminal.inject` and `copyTree.injectToTerminal` accept any panel id a listing returns, so granting them to that session let an agent in a read-only sandbox type into a neighbouring shell and run commands as the user. `RENDERER_OWNED_ORIGIN_ONLY_TOOLS` names them, and `getTierPermittedActionIds(tier, rendererOwnedOrigin)` returns `NON_RENDERER_OWNED_TIER_ALLOWLISTS[tier]` — the same surface with those removed — for every session that is not `help` or `assistant-pane`. Discovery, the call gate, `mcp.surface` and the target policy all read that one selector, and it defaults to the narrower set. At a tier that admits terminal input, such a session submits text or injects context into an open terminal only through `terminal.sendCommandOwned` and `terminal.injectOwned` (see [Resource ownership](#resource-ownership-11909)). Two launch arguments are refused alongside it, because either would hand back what the reservation takes away: `agent.launch` under a `requestedId` that names a live terminal (the spawn is refused with the original still running, so recording it would claim that shell), and `agent.launch` or `workflow.startWorkOnIssue` naming an assistant-only agent from a non-renderer-owned session, which would mint the assistant's pinned bearer. A recipe child of an assistant-only type is refused at launch for every caller, which closes the same route through `recipe.run`, `worktree.createWithRecipe` and a workflow's `recipeId`. That is not a sandbox — a session holding `terminal.new` can still open a shell and run commands in it — but it stops the authority reaching panels the session did not open. The assistant keeps the unscoped tools because sending prompts to agents the user launched is most of its work, and it runs pinned to the window the user is watching.
...
`requireExplicitWorktreeForAgentDispatch(actionId, args, ctx)` is the narrower form: the fallback stays for a person (palette, keybinding, context menu) and is refused only for `dispatchSource === "agent"`, which every MCP origin dispatches as. It reads the RAW selectors, so it must run BEFORE any resolution — `resolveWorktreeLocation` has already substituted the active worktree by the time it returns, and a guard placed after it could never tell an asserted target from an inherited one. Applied to `agentSessionHistory.resume`, `copyTree.generateAndCopyFile`, and `agent.launch`. The launch case is the sharpest: an omitted id resolves against the LIVE selection at the instant the call lands, launches fan out, and a batch dispatched while the user switches worktrees ends up split across two of them — with real terminals in the wrong place by the time any caller could re-read. A headless client must therefore name the worktree it means; see [`vendor/daintree-assistant/docs/DAINTREE_HOST.md`](../../vendor/daintree-assistant/docs/DAINTREE_HOST.md) for how the assistant pins one per turn and forwards it explicitly.
...
`worktree.waitUntilReady` is at the `workbench` tier and deliberately NOT on the external allowlist, which is at its tool-count ceiling. External callers get readiness through `setupState` on `worktree.list`, which is already on that surface.
...
All nine are in `WORKBENCH_TIER_TOOLS` (the help-assistant baseline), so they are reachable at `workbench` and above — but not at `external`. All are `kind:"query"`, `danger:"safe"`, on the `standard` bucket, and not deduped.
...
| `forge.openRepo` | safe | standard | no | `projectPath?` |
