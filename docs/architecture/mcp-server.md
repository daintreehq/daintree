...
Four tools **act** on the ledger, and none of them is the general primitive:
...
The generic `terminal.close` and `worktree.delete` stay off `MCP_EXTERNAL_TIER_TOOLS`, and so does `pilot.openRun` — it is reachable only through the `terminal.revealOwned` short-circuit. The same goes for `terminal.interrupt` and `fleet.interrupt`: the unscoped interrupts reach any pane a listing names, including the user's own mid-turn. All four `*Owned` tools execute as main-process short-circuits because the ledger is keyed by MCP session id, which the renderer never sees.
