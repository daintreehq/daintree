...
`terminal.list { owned: true }` reports that honestly rather than papering over it: after a reconnect it comes back empty, because the new session genuinely owns nothing. That is not the fix for a long-lived client that lost its claim — it is what lets one say so. Making ownership survive a reconnect means taking it off the transport session id entirely, and the credential shape that would do it is deliberately not this: one external API key is shared by every client (`mcpClientConfigs.ts`), so keying ownership off it would collapse every external client into a single owner — a wider boundary, not a narrower one.
