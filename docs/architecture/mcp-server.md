...
- **`exitCode` is the field that looks reachable and is not.** Main caches exit metadata, but `AgentAvailabilityStore` keys it by agent id, and an agent id names the agent _type_ (`claude`) rather than the spawn — `TerminalProcess` emits `launchAgentId` as the agent id, so several terminals share one and `agentToTerminal` keeps only the most recent. Joining through it would report whichever same-type terminal exited last, across workspaces; for a fleet of identical agents that is wrong far more often than right. So the `pty` path declares it unavailable, and `agentState` still separates `completed` from `exited`. (A separate, pre-existing inconsistency on the renderer path: it emits `exitCode: null` for a still-running terminal, which the schema reads as "signal-terminated with no numeric code". Left alone here — it predates this change and belongs to its own fix.)
