...
- `suspended` (**FUTURE_SAB skeleton**, #9900): only emitted by the SharedArrayBuffer transport path (`BackpressureManager.suspendVisualStream`, `electron/pty-host/backpressure.ts`). The SAB path is disabled in production (SharedArrayBuffer is not supported in Electron UtilityProcess — see PR #7724 / issue #7653). The renderer-side Suspended glyph and a11y formatter in `src/components/Terminal/TerminalStatusSlot.tsx` and `src/hooks/app/useAccessibilityAnnouncements.ts` remain as forward-looking code, but the status is never produced in production.
