...
| **Fleet** | `fleetArmingStore`, `fleetScopeFlagStore`, `fleetBroadcastProgressStore`, `fleetBroadcastConfirmStore`, `fleetFailureStore`, `fleetTargetOverridesStore`, `fleetPickerSessionStore`, `fleetResolutionPreviewStore`, `fleetPendingActionStore`, plus `fleetEligibility.ts` (pure helpers) | Largest single cluster by file count (~10). |
