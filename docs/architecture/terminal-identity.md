...
3. **Capability mode** — the agent capability surface this terminal is allowed to participate in (fleet membership, orchestration, hybrid input). Currently follows launch intent. Derived at read time; not persisted; reserved for future consumers. No production writer exists yet — the type slot is established so consumers can migrate without further IPC contract churn.

## Field mapping

| Field               | Surface                                                                                                                                                                                         | Concept                         | Type             | Writer(s)                                                                         | Persisted | Cleared on                              |
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------- | ---------------- | --------------------------------------------------------------------------------- | --------- | --------------------------------------- |
| `agentId`           | `PtyPanelData`, `TerminalInstance`, `TerminalPublicState`, `TerminalSpawnOptions`, `TerminalState`, `BackendTerminalInfo`, `TerminalReconnectResult`, `TerminalInfoPayload`                     | launch intent                   | `AgentId`        | spawn handler; `handleAgentDetection()` bridge write (known violation, see below) | yes       | never (until issue closes bridge write) |
...
| `type`              | `PtyPanelData`, `TerminalInstance`, `TerminalPublicState`, `TerminalSpawnOptions`, `TerminalState`, `BackendTerminalInfo`, `TerminalReconnectResult`, `TerminalInfoPayload`, `TerminalSnapshot` | legacy conflation               | `TerminalType`   | spawn handler, `handleAgentDetection()`                                           | yes       | — (deprecated; do not use for new code) |
...
- `electron/services/pty/TerminalProcess.ts` — `handleAgentDetection()` writes `terminal.agentId = result.agentType` on runtime detection (and clears it on agent exit). This violates the "launch intent is sealed" rule. It exists because `AgentStateService` routes state events by `agentId`, and runtime-promoted plain terminals need that routing to function. A follow-on issue will remove the bridge write by teaching `AgentStateService` to accept `detectedAgentId` as an alternate routing key.
...
- Use `capabilityAgentId` (when consumers start populating it) when asking: _"what is this terminal allowed to do?"_ This is the right field for fleet membership, orchestration gating, and hybrid input.
