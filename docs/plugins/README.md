...
Three things Daintree deliberately does **not** do at 1.0: it doesn't sign plugins or verify publisher identity (the SHA-256 archive hash confirms byte integrity, not origin); it doesn't gate installs behind a capability-consent dialog (capabilities are surfaced after install, in the plugin manager's detail pane); and it doesn't encrypt secret settings (`type: "secret"` values are plaintext JSON on disk, not in the OS keychain). The [trust model](./trust-model.md) documents these as the complete 1.0 non-guarantee contract.
