...
**Views in a window on another machine.** When the window is attached to a remote host, your view runs in that machine's renderer and its module is fetched from the host; your handlers run on the host. The link between them can drop: `useHostChannel` then reports `disconnected` and fails with `HostDisconnectedError` or `OutcomeUnknownError` ([Host API → `useHostChannel`](./host-api.md#usehostchannel--requestresponse-the-pull-half)), and a raw view sees the same errors as `code: "HOST_DISCONNECTED"` / `"OUTCOME_UNKNOWN"` on the rejected `invoke`. Relative imports inside your bundle keep working; an absolute `plugin://{pluginId}/…` URL you write by hand does not reach the host, so prefer relative ones. A view that can only work on the project's own machine declares [`"remote": "unsupported"`](./manifest.md#remote) and gets a placeholder there instead.
