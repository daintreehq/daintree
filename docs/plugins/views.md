...
- Module-scope state surviving a reload. Each plugin load — an install, a replacement install, an enable, or an app start — re-imports your bundle under a fresh generation; keep anything worth keeping in `persistState` (survives remounts and reloads) or `host.storage` (survives everything). The `daintree-plugin dev` hot-reload path inverts this — it respawns only the worker, so the view module and its state stay in memory and view edits do not appear mid-session (#12277); see [Contribution points → Worker reload vs. view-module replacement](./contribution-points.md#worker-reload-vs-view-module-replacement) for the mechanism and the workaround.
