...
- **Slot-replacing, never removing.** Surrounding chrome is untouched: the project switcher, the sidebar and the worktree dashboard stay where they are. The host draws a thin strip of its own above the region, outside the view's box, carrying the panel name, a "No panels open" label, the launcher's search entry and a switch back to the stock launcher, so a broken or half-finished surface cannot strand the user. Nothing overlaps the view, so it needs no reserved space.
