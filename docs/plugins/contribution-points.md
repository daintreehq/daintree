...
| `pluginId` | `string` | The host-side plugin identifier, taken from the panel kind's `extensionId`: the bare manifest `name` for an installed or builtin plugin, but the instance key `project__{projectId}__{manifestId}` for a project plugin. Pass it through to `window.electron.plugin.*` and the SDK hooks unchanged — a view that substitutes its own manifest name addresses nothing under project scope. Stable for the lifetime of the host, so it still serves for namespacing storage keys and log lines. |
| `worktreeId` | `string` \| `undefined` | The worktree the panel was spawned with, as recorded on the panel record. Read this rather than dispatching `worktree.getCurrent`, which answers for the _visible_ worktree and is wrong for a background or restored panel. `undefined` for a panel spawned without a worktree. |
...
**Where they appear.** Each field has exactly one home. An installed plugin's `user` fields are in the plugin manager, on its Settings tab; its `project` / `local` fields are in Project settings → Plugins, with the plugin selected; a project plugin's fields are all in Project settings. Each home shows only its own fields, with a row pointing to the other when there is one. Every way in lands in the right home: the **Plugin settings…** entry in your panels' ⋯ and right-click menus, the setup strip, [`host.settings.open(key?)`](./host-api.md#settings), and the `plugin.openSettings` action behind them, which scrolls to and briefly highlights `key` when it names a declared setting. A project-scoped destination with no project open is refused rather than sent somewhere the field isn't. `plugin.openSettings` is UI navigation: it is hidden from agents and MCP clients. The form applies each change as it is made; there is no Save.

**A custom section.** When fields aren't enough — a sign-in flow, a list editor, a connection test — declare one view with `location: "settings"` (see [Views](#views--shipped-panel-surface)). The host mounts it in the plugin's settings home below the generated fields, inside its own contained settings surface, and hands it `settingsContext: { scope, projectId }`: an installed plugin's section mounts with `scope: "user"` in the plugin manager and `scope: "project"` in Project settings, so render the rows for that scope. While the plugin is disabled or stopped, the section shows as a row saying it's available once the plugin runs.
