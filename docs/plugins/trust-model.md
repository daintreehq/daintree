...
- **No encrypted secret storage.** Settings declared `type: "secret"` are stored as plaintext JSON — user-scope values at `~/.daintree/plugin-settings/{pluginId}.json`, project-scope values at `<projectRoot>/.daintree/plugin-settings/{pluginId}.json` (the latter is tracked per-repo). The `secret` type masks the value in the UI and in log output only — it is not encryption. The user-scope file is created `chmod 0o600` on POSIX to restrict OS-level reads, but values are not encrypted at rest in either scope and are not kept in the OS keychain. Prefer scoped, short-lived, or read-only tokens for anything stored this way; don't store a credential that must survive disk compromise.
