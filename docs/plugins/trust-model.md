...
- **No encrypted secret storage.** Settings declared `type: "secret"` are stored as plaintext JSON at `~/.daintree/plugin-settings/{pluginId}.json`. The `secret` type masks the value in the UI and in log output only — it is not encryption. The file is created `chmod 0o600` on POSIX to restrict OS-level reads, but values are not encrypted at rest and are not kept in the OS keychain. Prefer scoped, short-lived, or read-only tokens for anything stored this way; don't store a credential that must survive disk compromise.
