...
What survives a reload: your `settings` values and everything in `host.storage`, because both are files keyed by identity rather than by process — and any panel state written with `PanelViewProps.persistState`, which is stored on the panel record alongside the layout rather than in the plugin process, so it outlives the worker entirely. What does not: module-scope state in your worker, and React state in your views. Design for a remount, exactly as for an installed plugin. [views.md](./views.md#what-survives-what) has the full matrix.
