...
What survives a reload: your `settings` values and everything in `host.storage`, because both are files keyed by identity rather than by process. What does not: module-scope state in your worker, and React state in your views. Design for a remount, exactly as for an installed plugin.
