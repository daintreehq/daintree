...
- **Slot-replacing, never removing.** The surrounding chrome is untouched: the project switcher, the sidebar and the worktree dashboard stay exactly where they were. Above the surface the host draws a thin strip of its own, laid out outside your view's box rather than over it: your panel name, a "No panels open" label so the region reads as the empty canvas, the launcher's search entry, and a switch between your surface and the stock launcher in both directions. The launcher is always one click away, a half-finished surface can never strand the user, and your view never needs to leave room for host controls.
