...
| D10 | **Plugin agent detection: cut the `detection` field for 1.0 (minimal tier) vs ship the full-tracking tier.** **Resolved — ship (#10587):** the `detection` field was reinstated as an optional manifest block (after an initial cut in #10460); a plugin agent that declares output-pattern regexes with confidence weights drives the live agent-state UI through the pty-host activity monitor, just like built-in agents. Agents that omit `detection` still launch as named, untracked terminals. | Determines whether a heavily-validated manifest field stays or goes. |
...
1. **Schema validates fields the runtime ignores (frozen-promise violations).** The single largest freeze-risk class. `scopes.network.allowedUrls` enforcement, `ForgeProviderContribution.capabilities`, and `ViewContribution.location:"sidebar"` are all validated at manifest-parse time but unconsulted or outright rejected at runtime. A plugin author writes a manifest, it passes validation, and nothing happens. Freezing these means freezing promises we don't keep. (Plugin agent `detection` was once the canonical example — cut in #10460 — but it has since been reinstated and fully wired to the pty-host activity monitor in #10587, so it is no longer a validated-but-ignored field. `scopes.fs.allowedPaths` is now **runtime-enforced for the new host-mediated `host.fs`/`host.git` surface** — realpath containment, traversal/symlink-escape rejected — so it is no longer purely advisory for that path; the residual gap is the in-process `node:fs` escape, which is a D3 sandbox concern, not a field-vs-runtime divergence.)
...
| --- | --- | --- | --- | --- |
| 1 | ~~Resolve plugin agent detection: wire it or cut it from the 1.0 schema (D10)~~ **Resolved — wired (#10587, after an initial cut in #10460)** | decision | high | large |
...
| 8 | Reconcile `scopes` (`fs.allowedPaths`, `network.allowedUrls`) with the no-enforcement reality across schema, docs, UI (D4) — **`fs.allowedPaths` now runtime-enforced for `host.fs`/`host.git` via realpath containment (out-of-scope paths reject with `PATH_NOT_ALLOWED`); `network.allowedUrls` remains advisory-only (capability-lattice elevation, not a request filter), and host-mediated network is deferred post-1.0** | decision | high | medium |
...
| `agents` | Launch wired; `detection` optional and wired to the pty-host activity monitor (#10587) | Freeze |
...
- **`host.process.spawn(command, opts)`** — a managed, supervised child-process surface (kill/restart/onExit/onCrash, per-plugin concurrency cap, lifecycle-tied teardown), gated on the declared `shell:exec` capability — one of the runtime-enforced scope capabilities alongside the `fs:*` / `git:*` gates on the `host.fs` / `host.git` surface below.
