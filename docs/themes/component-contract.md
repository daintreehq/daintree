...
496 sites stay dim on purpose — icon affordances, decorative glyphs, deliberate disabled states, text already composited by an ancestor's `opacity-*`, and controls whose resting colour sits beside a semantic state colour it would otherwise erase. Each is recorded with its category in `scripts/baselines/text-ramp-manifest.json`, generated by `npm run theme:text-ramp`, and a contract test fails if a ramp site appears that the manifest does not name.
