...
- `brand-mark-saturation` — a bare saturation multiplier applied by `.brand-mark` in `src/index.css` to third-party brand marks (agent and product logos). Those carry vendor hexes from `AgentConfig.color`, not theme tokens, so they are the one colour family the semantic palette cannot reach; a theme running a contrast budget can damp them so a row of vendor logos stops out-shouting its own signals. Full brand colour returns on hover **or keyboard focus** of the containing control, and on a selected `role="option"` row — restoring on hover alone stranded keyboard users at the damped saturation. The CSS fallback is `1`, and `BrandMark.tsx` attaches the hook class only when the active scheme sets the key, so themes that ignore it render byte-for-byte unchanged and pay no filter. Set by movile (`0.4`, a contrast-budget theme) and daintree (`0.65`, the default).
