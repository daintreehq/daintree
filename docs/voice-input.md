...
- `buildCorrectionSystemPrompt(context)` — layers project context, the user's custom dictionary (REQUIRED TERMS), and custom instructions, ending with a fixed guardrail suffix. Ordered fixed-first so the OpenAI prompt cache (`prompt_cache_key`, prefix `voice-correction-v6`) hits. The fixed `CORE_CORRECTION_PROMPT` prefix is kept above OpenAI's 1,024-token cache floor, and `VoiceCorrectionService` sends it as the first `developer` message in the `input` array (not the top-level `instructions` field) so the prefix is actually cache-eligible.
...
`voiceContextKeyterms.ts` (`assembleKeyterms`, `formatKeytermPrompt`, branch/project/terminal tokenizers) builds a dynamic keyterm prompt from the active branch, project name, and terminal output. It is **infrastructure that is not yet wired into the live provider pipeline** (only its own test imports it) — the `TODO: pass transcription.prompt … once #7832 lands` in `OpenAITranscriptionProvider` is the intended hookup point. Note that when reading the current tree.
