...
- `buildCorrectionSystemPrompt(context)` — layers project context, the user's custom dictionary (REQUIRED TERMS), and custom instructions, ending with a fixed guardrail suffix. Ordered fixed-first so the OpenAI prompt cache (`prompt_cache_key`, prefix `voice-correction-v5`) hits.
