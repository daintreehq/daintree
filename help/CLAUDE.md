...
2. Give the loop one pacing owner: `ScheduleWakeup`, or a `terminal.registerWatch` pane watch if that tool is available and accepts your pane. Never stack a second timer, background `sleep`, monitor or polling script on top: two wakers mean duplicate checks and double actions.
...
4. Each wake, one batched `terminal.getStatus` over the running ids. A job is done when its agent is waiting and its PR exists. `prNumber`/`prUrl` in `worktree.list` is a cached hint, so confirm with `forge.getPR` (or `forge.listPRs` when it is null) and read the agent's final report (`terminal.readLastMessageOwned` for Claude Code). Don't scrape a PR number from the agent's screen or write your own poller.
