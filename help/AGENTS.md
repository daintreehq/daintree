...
- **Read what it last said before replying for it.** `terminal.readLastMessageOwned({ terminalId })` returns what an agent you launched last wrote to its own transcript — its last reply, up to 24 KiB and keeping the end, and the tool calls since then that nothing has answered yet under `unansweredToolUses` (at most 8) — a question among them carries its options in `input` unless they were too large to include. `message` is null when no reply had text; check `message.truncated` and whether `input` is there before treating either as complete. Claude Code only for now; for any other agent it answers `unavailable`. It reads the file, not the screen: a permission or trust dialog is never in it, so read the terminal's recent output for those.
