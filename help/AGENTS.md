...
Under Codex, this session's transcript is the JSONL file for `CODEX_THREAD_ID` under `$CODEX_HOME/sessions` (default `~/.codex/sessions`); give its absolute path when asked.
