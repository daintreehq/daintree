...
| `system` | User selects "System — destructive and external writes" in Settings → Assistant | Adds worktree creation at an explicit root outside the project, terminal arm/disarm for automation, git stage/unstage/fetch/commit/push, clipboard export, and forge issue/PR/review reads and writes from the local app. |
