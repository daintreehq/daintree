...
describe("MCP wire budget — aggregate ratchets (§9)", () => {
  ...
  // the tool exists to tell four per-target outcomes apart — destroyed,
  // deselected by the approver, already gone, newly busy — and a caller that
  // cannot distinguish them will retry the ones a human just refused. Naming
  // them is the tool, not decoration on it.
