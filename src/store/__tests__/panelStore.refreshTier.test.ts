...
describe("getTerminalRefreshTier - fleet-aware demotion (#8596)", () => {
  // When multiple agents are simultaneously working, only the focused pane
  // keeps the FOCUSED top-tier. Sibling working agents drop to VISIBLE so the
  // renderer budget isn't split N ways at 60fps.
  const workingAgent = (): TerminalInstance =>
    makeTerminal({
      kind: "terminal",
      launchAgentId: "claude",
      detectedAgentId: "claude",
  ...
  it("demotes an unfocused working agent to VISIBLE when another agent is also working", () => {
    expect(getTerminalRefreshTier(workingAgent(), false, { workingCount: 2 })).toBe(
      TerminalRefreshTier.VISIBLE
    );
  });

  it("demotes an unfocused working agent to VISIBLE for any workingCount > 1", () => {
    expect(getTerminalRefreshTier(workingAgent(), false, { workingCount: 5 })).toBe(
      TerminalRefreshTier.VISIBLE
    );
  });

  it("keeps the focused working agent at FOCUSED even when siblings are also working", () => {
    expect(getTerminalRefreshTier(workingAgent(), true, { workingCount: 3 })).toBe(
      TerminalRefreshTier.FOCUSED
    );
  });

  it("keeps a lone working agent at FOCUSED when unfocused (workingCount=1)", () => {
    // Lone worker keeps the top tier regardless of focus — there's no fleet
    // pressure to balance against.
    expect(getTerminalRefreshTier(workingAgent(), false, { workingCount: 1 })).toBe(
      TerminalRefreshTier.FOCUSED
    );
  });

  it("keeps a working agent at FOCUSED when workingCount is omitted (back-compat default)", () => {
    // Callers without fleet context (HelpPanel, tests, pre-#8596 paths) get
    // the old single-terminal behavior.
    expect(getTerminalRefreshTier(workingAgent(), false)).toBe(TerminalRefreshTier.FOCUSED);
  });

  it("keeps a working agent at FOCUSED when workingCount is 0 (defensive)", () => {
    // A 0 count means the caller hasn't seen the new working agent yet —
    // don't demote based on a count that's stale relative to this terminal.
    expect(getTerminalRefreshTier(workingAgent(), false, { workingCount: 0 })).toBe(
      TerminalRefreshTier.FOCUSED
    );
  });

  it("does not demote a non-working agent even when workingCount > 1", () => {
    ...
    // Idle agents go through the runtime-agent VISIBLE rule anyway, but this
    // confirms the working-only fleet guard doesn't accidentally widen.
    expect(getTerminalRefreshTier(idleAgent, false, { workingCount: 3 })).toBe(
      TerminalRefreshTier.VISIBLE
    );
  });
