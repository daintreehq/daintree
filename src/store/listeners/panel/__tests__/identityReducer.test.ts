...
it("does not roll the session count backwards", () => {
  // A detection event overtaken by a later one must not make a session that
  // has already been superseded look current again.
  const terminal = makeTerminal({
    detectedAgentId: "claude",
    detectedProcessId: "claude",
    everDetectedAgent: true,
