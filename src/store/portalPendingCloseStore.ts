 ...
 * (`closeAllTabs` / `closeOthers` / `closeToRight` at the 3+-tab threshold).
 * The action `run()` writes here instead of closing immediately; the app-level
 ...
 */
export type PortalPendingCloseKind = "closeAll" | "closeOthers" | "closeToRight";
...
export interface PortalPendingCloseSnapshot {
  ...
  /**
   * For `closeOthers`: the tab to keep. For `closeToRight`: the anchor tab
   * whose right-hand neighbours close (the anchor itself is kept). The
   * confirmed dispatch carries this so the action re-derives the close set
   * from current store state rather than the (possibly stale) snapshot.
   */
  keepTabId?: string;
}
