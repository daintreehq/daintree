...
interface HostMetricsState {
  /** Per host, newest first. "local" is this machine. */
...
export const useHostMetricsStore = create<HostMetricsState>()((set) => ({
  ...
  seed(snapshots) {
    set((state) => {
      ...
      for (const snapshot of snapshots) {
        ...
        const live = history[snapshot.hostId] ?? [];
        ...
        history[snapshot.hostId] = [...fresher, ...seeded].slice(0, HOST_METRICS_HISTORY_SIZE);
      }
      return { history };
    });
  },
  apply(summary) {
    set((state) => {
      const ring = state.history[summary.hostId] ?? [];
      ...
      return {
        history: {
          ...state.history,
          [summary.hostId]: [summary, ...ring].slice(0, HOST_METRICS_HISTORY_SIZE),
        },
      };
    });
  },
  forget(hostId) {
    set((state) => {
      if (!(hostId in state.history)) return state;
      const history = { ...state.history };
      delete history[hostId];
      return { history };
    });
  },
  reset() {
