...
export function KiroIcon({ className, size = 16, ...props }: KiroIconProps) {
  // Several Kiro marks render at once (dock rows, palettes, the agent grid) and
  // duplicate DOM ids make every `url(#…)` resolve to whichever mask landed in
  // the document first, so the id has to be per-instance.
  const maskId = useId();
  return (
    <svg
      width={size}
    ...
    >
      {/*
        The mark used to draw a coloured tile, a white mascot and black pupils.
        Nothing hands a colour to a glyph any more — the mascot is a single
        inherited ink — so the pupils survive as negative space masked out of it
        rather than as a second colour a one-ink glyph has no way to hold. The
        black and white inside the mask are geometry (hide/show), not paint.
      */}
      <mask
        id={maskId}
