...
export function KiroIcon({ className, size = 16, ...props }: KiroIconProps) {
  // The mask id has to be unique per instance: several Kiro marks render at once
  // (dock rows, palettes, the agent grid) and duplicate ids make `url(#…)` pick
  // whichever landed in the document first.
  const maskId = useId();
  return (
    <svg
      width={size}
    ...
    >
      {/*
        The mark used to draw its own tile, a white mascot and black pupils; the
        badge paints the tile now (#11895) and the mascot is a single inherited
        ink. The pupils survive as negative space — masked out of the mascot so
        the tile shows through — rather than as a second colour, which a knocked
        out glyph has no way to hold.
      */}
      <mask
        id={maskId}
