...
export function KiroIcon({ className, size = 16, brandColor, ...props }: KiroIconProps) {
  return (
    ...
    >
      {/*
        With a brandColor we draw the full mark: colored tile, white mascot, black
        pupils. Without one (the muted/disabled state) no single solid color can
        desaturate the hardcoded white/black internals, so we drop the tile and
        pupils and render just the mascot silhouette in currentColor — a clean
        single-token glyph consistent with every other muted agent icon. See #10458.
      */}
      {brandColor && <rect width="1200" height="1200" rx="260" fill={brandColor} />}
      <mask
        id="kiro-mask"
      ...
      <g mask="url(#kiro-mask)">
        <path
          ...
          fill={brandColor ? "white" : "currentColor"}
        />
        {brandColor && (
          <>
            <path
