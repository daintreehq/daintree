...
describe("Toast severity-based dismissal (issue #5859)", () => {
  ...
  // The toaster's previous DEV-only "non-string message without inboxMessage"
  // mirror has been removed — `NotifyPayload`'s discriminated union makes
  // that shape a compile error at the `notify()` call site, so the renderer
  // no longer needs a runtime backstop.
});
...
describe("Toast stack motion (issue #9618)", () => {
  ...
  it("assigns ascending stack indices, newest frontmost", async () => {
    ...
    expect(toasts[2].style.getPropertyValue("--toast-index")).toBe("2");
  });
