...
describe("Tooltip wrapper — hoverable content (WCAG SC 1.4.13)", () => {
  beforeEach(() => {
    rootSpy.mockClear();
  });

  function lastRootProps(): Record<string, unknown> {
    return rootSpy.mock.calls.at(-1)![0] as Record<string, unknown>;
  }

  it("lets the pointer onto a tooltip whose body is the content", () => {
    render(
      <Tooltip autoDismiss={false}>
        <TooltipTrigger>t</TooltipTrigger>
      </Tooltip>
    );
    expect(lastRootProps().disableHoverableContent).toBe(false);
  });

  it("leaves a transient hint to the provider's pass-through default", () => {
    render(
      <Tooltip>
        <TooltipTrigger>t</TooltipTrigger>
      </Tooltip>
    );
    expect(lastRootProps().disableHoverableContent).toBeUndefined();
  });

  it("honours an explicit prop over the autoDismiss-derived default", () => {
    render(
      <Tooltip autoDismiss={false} disableHoverableContent>
        <TooltipTrigger>t</TooltipTrigger>
      </Tooltip>
    );
    expect(lastRootProps().disableHoverableContent).toBe(true);
  });
});
