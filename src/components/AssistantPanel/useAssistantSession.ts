...
export interface AssistantSessionOptions {
  ...
  enabled: boolean;
  /**
   * Which parallel lane this session is (#12108).
   *
   * Decides two things together, which is why it is one input rather than two: the
   * engine's state namespace in main — and therefore which conversation the child
   * opens — and which per-lane store the events reduce into here. Splitting them would
   * make it possible to point a lane's UI at another lane's engine.
   */
  slot?: number;
...
/**
 * How many sessions the gap will hold frames for at once.
 *
 * The buffer is bucketed by session because every lane's hook watches ONE event channel
 * and cannot tell its own frames from a sibling's until `start()` resolves. A flat
 * buffer let a lane that was mid-answer spend the whole cap on its own tokens, and the
 * lane actually cold-starting then lost its own pre-ready frames to an eviction it had
 * no part in. Bucketing costs nothing and removes the interference; this cap only stops
 * a long-stalled start from accumulating a bucket per session id it ever saw.
 */
const MAX_PRE_ADOPTION_SESSIONS = 8;
 ...
 */
function isCommandInput(text: string, store: AssistantStoreApi): boolean {
  ...
  if (store.getState().commands.some((c) => c.name.replace(/^\//, "").toLowerCase() === name)) {
    return true;
  }
...
export function useAssistantSession(opts: AssistantSessionOptions): AssistantSessionActions {
  // Resolved during render, not in an effect: the store is this hook's whole state, and
  // an effect-assigned one would leave the first render — including the reset below —
  // writing into the wrong lane. `assistantStoreForSlot` is idempotent and its result is
  // stable for the life of the lane, so this is a lookup rather than a creation.
  const store = assistantStoreForSlot(opts.slot ?? DEFAULT_ASSISTANT_SLOT);
  ...
  const preAdoptionRef = useRef<Map<string, AssistantHostEvent[]>>(new Map());
  ...
  useEffect(() => {
    const offEvent = window.electron.assistantHost.onEvent((event) => {
      if (event.sessionId !== sessionIdRef.current) {
        ...
        if (sessionIdRef.current === null && pendingSessionRef.current !== null) {
          const held = preAdoptionRef.current.get(event.sessionId);
          if (held) {
            if (held.length < MAX_PRE_ADOPTION_FRAMES) held.push(event);
          } else if (preAdoptionRef.current.size < MAX_PRE_ADOPTION_SESSIONS) {
            preAdoptionRef.current.set(event.sessionId, [event]);
          }
        }
        return;
      }
      ...
      if (event.type === "approval:requested") {
        const covered = store.getState().consumeGrant({
    ...
    const offPeerPrompt = window.electron.assistantHost.onPeerPrompt((payload) => {
      ...
      store.getState().appendUserTurn(payload.text);
    });

    const offGap = window.electron.assistantHost.onSequenceGap((payload) => {
      if (payload.sessionId !== sessionIdRef.current) return;
      store.getState().recordGap(payload.missing);
    });
  ...
  }, [flushTokens, scheduleFlush, send, store]);

  const { projectId, cwd, enabled, restartNonce = 0, slot = DEFAULT_ASSISTANT_SLOT } = opts;
    ...
    safeFireAndForget(
      window.electron.assistantHost
        ...
        .then((started) => {
          ...
          if (ready) store.getState().applyEvent(ready);
          else store.getState().setConnection("ready", null);
          ...
          store.getState().setMcpUnavailable(mcpUnavailableReason ?? null);
          ...
          const flushPromptsBefore = (seq: number) => {
            while (pendingPrompts.length > 0 && (pendingPrompts[0]?.afterSeq ?? 0) < seq) {
              ...
              if (prompt) store.getState().appendUserTurn(prompt.text);
            }
          };
          ...
          for (const event of preAdoptionRef.current.get(sessionId) ?? []) {
            if (seen.has(event.seq)) continue;
            store.getState().applyEvent(event);
          }
          // Anything the user asked after the last event we have.
          for (const prompt of pendingPrompts) {
            store.getState().appendUserTurn(prompt.text);
          }
          preAdoptionRef.current = new Map();
          pendingSessionRef.current = null;
  ...
  const submit = useCallback(
    (text: string): boolean => {
      ...
      if (store.getState().connection !== "ready") return false;
      ...
      // the model.
      if (isCommandInput(text, store)) {
        const commandTurnId = store.getState().appendUserTurn(text);
        ...
        safeFireAndForget(
          window.electron.assistantHost
            ...
            .then(({ delivered }) => {
              ...
              if (commandTurnId) state.dropLocalTurn(commandTurnId);
              else state.dropUndeliveredText(text);
              state.pushNotice(
                "error",
                "That command didn't reach the assistant — it has stopped."
              );
            })
        );
        return true;
      ...
      safeFireAndForget(
        window.electron.assistantHost
          ...
          .then(({ delivered }) => {
            ...
            if (localTurnId) state.dropLocalTurn(localTurnId);
            else state.dropUndeliveredText(text);
            state.pushNotice("error", "That message didn't reach the assistant — it has stopped.");
          })
      );
      return true;
    },
  ...
  const cancelTimer = useCallback(
    (timerId: string) => {
      const sessionId = sessionIdRef.current;
      if (!sessionId) return;
      store.getState().markTimerCancelPending(timerId);
