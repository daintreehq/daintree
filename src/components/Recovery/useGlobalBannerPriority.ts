...
//   github-token       — GitHub credentials expired; auth failure, panel data broken
...
//
// github-token and cloud-sync sit below the recovery block. restore-confirmation
// stays above github-token because its auto-dismiss timer only runs while the
// banner is mounted, so it must keep that window when both conditions coexist.
// github-token outranks cloud-sync because an expired token is an active
// failure (GitHub data is broken now) whereas cloud-sync is a persistent
...
export function useGlobalBannerPriority(): GlobalBannerSlot {
  ...
  const tokenUnhealthy = useGitHubTokenHealthStore((s) => s.isUnhealthy);
  ...
  if (tokenUnhealthy) return "github-token";
