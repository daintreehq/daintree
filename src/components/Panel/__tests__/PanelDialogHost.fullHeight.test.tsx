...
const captured = vi.hoisted(() => ({ className: undefined as string | undefined }));
...
// Render one kind as a single-frame dialog and return whatever className the
// host passed AppDialog for it (undefined when the kind isn't pinned).
function classNameFor(kind: string): string | undefined {
  const id = `${kind}-1`;
  panelsById.current = { [id]: { id, kind, title: kind } };
  captured.className = undefined;
  usePanelDialogStore.setState({ dialogStack: [], requestSeq: 0 });
  const { unmount } = render(<PanelDialogHost />);
  act(() => {
    usePanelDialogStore.setState({ dialogStack: [id] });
  });
  const result = captured.className;
  unmount();
  return result;
}
