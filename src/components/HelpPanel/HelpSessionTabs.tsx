...
interface SessionTabChipProps {
  tab: HelpSessionTab;
  /**
   * The lane's state, already resolved. Deliberately a separate prop rather than
   * read off `tab`: it is the seam that lets a lane whose state lives somewhere
   * other than the tab — its own store, say — wrap this chip in a component that
   * subscribes and passes the result down. That wrapper is the only legal way to
   * subscribe to a variable number of stores, and it keeps the read narrow enough
   * that one background lane's activity re-renders one chip, not the strip.
   */
  agentState: AgentState | null | undefined;
  isActive: boolean;
...
function SessionTabChip({ tab, agentState, isActive, onSelect, onClose }: SessionTabChipProps) {
  const stateId = useId();

  return (
    <div
      // `session-tab` is not styling — it is the handle for the selected tab's
      // rail and its forced-colors fallback, both in `index.css`. `relative`
      // because the rail is a `::after` positioned against this element, and
      // `data-active` because the mark belongs to the whole chip while
      // `aria-pressed` belongs to the selector inside it.
      ...
      className={cn(
        ...
        // `overlay-raised` is the app's selection fill — the same one a palette
        // row and a highlighted menu item wear. It replaced `overlay-subtle`,
        // which is 2% and was the whole of why the selected session was
        // unreadable. It still cannot carry the signal alone; the rail below it
        // is what does.
    ...
    >
      <button
        ...
        // identifying tail cannot truncate, and the accessible-name algorithm
        // trims each element's own contribution before joining them — which
        // silently turned "Session 1" into "Session1" for every screen reader.
        // The name is not a presentation detail and must not depend on how the
        // text is broken up to fit.
        aria-label={tab.label}
        // The state reaches assistive tech as a DESCRIPTION, not as part of the
        // name. Stating the name above was necessary, but an explicit label on a
        // button also overrides everything inside it, which took the marker's
        // meaning away from exactly the reader who cannot see the glyph.
        aria-describedby={agentState ? stateId : undefined}
        onClick={() => onSelect(tab.slot)}
        className={cn(
          // `pl-2` against the strip's own `px-1` puts this tab's first ink at
          // 12px — the same column the header's Daintree mark starts in, one row
          // up. At `pl-2` inside `px-2` the strip began 4px inboard of the header
          // and the two bars shared no left edge.
          //
          // The rest of the spacing is cut to the bone on purpose, and the
          // constraint is the 320px minimum panel width: three lanes, three 24px
          // close targets and three 14px markers leave about 55px for each label,
          // which is what `Session N` measures. Every pixel spent on padding here
          // comes straight out of the label.
          //
          // `pr-1` is not symmetry — it is the inset focus ring's clearance. The
          // ring is drawn 1px inside this button and is 2px thick, so with no
          // right padding it painted over the label's last character, which on a
          // `Session N` label is the only character that identifies the lane.
          "flex items-center gap-1.5 min-w-0 pl-2 pr-1 py-1",
          "text-xs rounded-[var(--radius-sm)] transition-colors duration-150 ease-out",
          // Inset, unlike every other ring in this panel. The chip is 24px in a
          // strip whose padding is 4px, so an outset 2px ring at a 2px offset
          // measured exactly 32px — flush against the header's hairline above and
          // the body's below, with nothing between them, and on the first tab it
          // also sat 2px off the panel's own edge. Drawn inside, it reads as a
          // ring on the selector instead of a second divider.
      ...
      >
        ...
        {/* No `max-w`. The old `max-w-[10rem]` was 160px against a label that is
            always `Session N` and never exceeds 57px, so it could not fire; what
            the label actually needs is to give way when three lanes meet the
            320px minimum, which is a min-width problem, not a max-width one. */}
        <TabLabel label={tab.label} />
      </button>
      <button
        type="button"
        onClick={() => onClose(tab.slot)}
        // Always in the DOM so the control is reachable by keyboard and by
        // assistive tech; only its paint is gated. Shown outright on the selected
        // tab — that is the session you would close, and a control the pointer
        // has to go looking for is one most people never find — and hover/focus-
        // gated on the rest, which keeps a strip of three from reading as a row
        // of dismiss buttons.
        className={cn(
          // 24x24, the WCAG 2.2 SC 2.5.8 floor, reached by growing the BOX and
          // leaving the 12px glyph alone. It was a 16px target — `p-0.5` around
          // that same glyph — butted directly against the selector, so the
          // spacing exception did not apply either and a miss opened the lane you
          // were trying to close. The chip is exactly 24px tall, so this costs
          // height nothing.
          ...
          // Inset for the same reason as the selector's, and for one more: an
          // outset ring on a control this close to the label overlapped the
          // label's last character.
          "focus-visible:outline focus-visible:outline-2 focus-visible:outline-accent-primary focus-visible:-outline-offset-1"
        )}
