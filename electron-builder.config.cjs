...
module.exports = async function () {
  ...
  return {
    ...
    // Strips node-gyp's per-arch .deps/obj.target intermediates, which would
    // otherwise abort the macOS universal merge. See the file for why globs
    // can't do this reliably.
    beforePack: "./scripts/beforePack.cjs",
    afterPack: "./scripts/afterPack.cjs",
    afterSign: "./scripts/notarize-macos.cjs",
    mac: {
      // node-gyp's .deps/obj.target intermediates are regenerated by the
      // per-arch native rebuild that runs after `beforePack` has stripped
      // them, so one arch's ASAR can still carry a copy the other lacks.
      // @electron/universal then fails the merge on a unique file with no
      // allowList rule. Nothing reads these — a text dependency list and
      // object files — so permitting the asymmetry is safe. `beforePack`
      // still removes them wherever it wins the race, which is what keeps
      // the harder "can't reconcile two non-macho files" error away.
      singleArchFiles:
        "**/build/{Makefile,binding.Makefile,config.gypi,gyp-mac-tool,*.mk,Release/{.deps,obj.target}{,/**},Release/.forge-meta}",
      extraResources: [
